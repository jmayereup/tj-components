...
class O extends HTMLElement {
  ...
  render(t) {
    ...
    e.innerHTML = `<style>${_}</style><style>${P}</style>${M}`, this.shadowRoot.firstChild && (this.shadowRoot.innerHTML = ""), this.shadowRoot.appendChild(e.content.cloneNode(!0)), t.title && (this.shadowRoot.getElementById("pronunciationTitle").textContent = t.title);
    let o = "";
  ...
  renderListenRecord(t, e) {
    return `
            <div class="tj-card" id="act-${e}">
                <div class="activity-title tj-h3">${S.headphones} Listen & Record</div>
  ...
  attachEventListeners() {
    ...
    const r = this.shadowRoot.getElementById("show-report-btn"), a = this.shadowRoot.getElementById("report-overlay"), s = this.shadowRoot.getElementById("cancel-report-btn"), l = this.shadowRoot.getElementById("generate-btn"), u = this.shadowRoot.getElementById("rc-close-btn"), f = this.shadowRoot.getElementById("submit-score-btn");
    r && (r.onclick = () => this._showReportOverlay()), s && (s.onclick = () => this._hideReportOverlay()), a && (a.onclick = (d) => {
      d.target === a && this._hideReportOverlay();
    }), u && (u.onclick = () => this._hideReportOverlay()), l && (l.onclick = () => this._generateReport()), f && (f.onclick = () => this._submitScore()), this.shadowRoot.querySelectorAll(".translation-toggle").forEach((d) => {
      ...
      d.addEventListener("click", (n) => {
        ...
        if (m === g) {
          ...
          const R = n.target.closest(".scramble-container"), x = R.querySelector(".scramble-controls"), v = R.querySelector(".play-audio-btn");
          x && (x.style.display = "none"), v && (v.style.display = "none"), c && (c.style.display = "none");
          const j = n.target.closest(".tj-card");
          j && (j.classList.add("completed"), this.updateProgress()), setTimeout(() => {
            p.textContent = "Activity Completed ✓", c.querySelectorAll(".scramble-word").forEach((C) => C.style.display = "none");
            const k = this.shadowRoot.querySelector("#bank-" + i);
            k && k.querySelectorAll(".scramble-word").forEach((E) => E.style.display = "none");
          }, 3e3);
        } else
  ...
  _generateReport() {
    ...
    const f = this.shadowRoot.querySelectorAll(".tj-card[id^='act-']").length, d = this.shadowRoot.querySelectorAll(".tj-card.completed").length, n = Math.round(d / f * 100) || 0, i = (/* @__PURE__ */ new Date()).toLocaleString();
    let c = "🏆";
    n < 50 ? c = "💪" : n < 80 && (c = "⭐");
    const h = `
          <div class="rc-header">
          ...
          <div class="rc-score-row">
              <div class="rc-score-circle">
                  <div class="rc-score-val">${d}/${f}</div>
          ...
          <div class="rc-details">
              <div class="rc-detail-row"><span>Total Completed</span><span>${d} / ${f} activities</span></div>
              <div class="rc-detail-row"><span>Completed On</span><span>${i}</span></div>
      `, g = this.shadowRoot.getElementById("initial-form"), b = this.shadowRoot.getElementById("report-area"), m = this.shadowRoot.getElementById("submit-actions"), R = this.shadowRoot.getElementById("report-teacher-code");
    if (R && (R.value = u), g && (g.style.display = "none"), b && (b.style.display = "block", b.innerHTML = h, this.recordings.size > 0)) {
      const x = document.createElement("div");
      x.classList.add("recordings-section"), x.style.marginTop = "20px", x.style.textAlign = "left";
      const v = document.createElement("h4");
      v.style.display = "flex", v.style.alignItems = "center", v.style.gap = "8px", v.style.margin = "0 0 12px 0", v.style.color = "var(--tj-text-main)", v.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="var(--tj-primary)"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg> Student Recordings', x.appendChild(v);
      const j = document.createElement("div");
      j.classList.add("recordings-list"), j.style.display = "flex", j.style.flexDirection = "column", j.style.gap = "8px", Array.from(this.recordings.keys()).sort((k, C) => k - C).forEach((k) => {
        const C = this.shadowRoot.getElementById(`act-${k}`);
        let E = "Recording " + (parseInt(k) + 1);
        if (C) {
          const B = C.querySelector(".lr-target-word");
          B && (E = B.textContent);
        }
        const y = document.createElement("div");
        y.classList.add("recording-item"), y.style.display = "flex", y.style.alignItems = "center", y.style.gap = "12px", y.style.padding = "8px", y.style.background = "var(--tj-bg-main)", y.style.border = "1px solid var(--tj-border-main)", y.style.borderRadius = "8px";
        const w = document.createElement("button");
        w.classList.add("tj-icon-btn", "recording-play-btn"), w.style.width = "32px", w.style.height = "32px", w.style.padding = "0", w.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>', w.title = "Play Recording", w.onclick = () => this._playReportRecording(k, w);
        const L = document.createElement("div");
        L.classList.add("recording-text"), L.style.fontSize = "0.9em", L.style.color = "var(--tj-text-main)", L.textContent = E, y.appendChild(w), y.appendChild(L), j.appendChild(y);
      }), x.appendChild(j), b.appendChild(x);
    }
  ...
  async _submitScore() {
    ...
    const s = this.shadowRoot.querySelectorAll(".tj-card[id^='act-']").length, l = this.shadowRoot.querySelectorAll(".tj-card.completed").length, u = this.shadowRoot.getElementById("pronunciationTitle").textContent || "Pronunciation Practice", f = {
      ...
      for (let s = 0; s < 2; s++) {
        for (const l of t) {
          const u = Array.from(a).find(
            (f) => f.textContent.trim() === l.trim()
          );
          u && u.classList.add("highlight"), await this.playTTS(l, o), u && u.classList.remove("highlight"), await new Promise((f) => setTimeout(f, 600));
        }
