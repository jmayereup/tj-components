    ...
    <div class="report-overlay" id="report-overlay" style="display:none;">
        <div class="report-modal">
            <div class="initial-form" id="initial-form">
                ...
                <input type="text" id="homeroom-input" placeholder="Homeroom" autocomplete="off">
...
class M extends HTMLElement {
  ...
  attachEventListeners() {
    ...
    const r = this.shadowRoot.getElementById("show-report-btn"), a = this.shadowRoot.getElementById("report-overlay"), i = this.shadowRoot.getElementById("cancel-report-btn"), l = this.shadowRoot.getElementById("generate-btn"), u = this.shadowRoot.getElementById("rc-close-btn"), y = this.shadowRoot.getElementById("submit-score-btn");
    r && (r.onclick = () => this._showReportOverlay()), i && (i.onclick = () => this._hideReportOverlay()), a && (a.onclick = (d) => {
      ...
      d.addEventListener("click", (n) => {
        const s = n.target.dataset.index, c = this.shadowRoot.querySelector("#trans-" + s);
      ...
      d.addEventListener("click", (n) => {
        const s = n.target.closest("button"), c = s.dataset.text, p = s.dataset.index;
        this.playTTS(c, s).then(() => {
          if (p !== void 0) {
      ...
      d.addEventListener("click", (n) => {
        const s = n.target.closest("button"), c = s.dataset.options.split(","), p = s.dataset.index, h = c[Math.floor(Math.random() * c.length)], g = s.closest(".mp-container");
        ...
        b && (b.textContent = "", b.className = "feedback-msg"), this.playMinimalPairSequence(c, h, s);
      });
    }), this.shadowRoot.querySelectorAll('button[data-action="record"]').forEach((d) => {
      d.addEventListener("click", async (n) => {
        const s = n.target.closest("button"), c = s.dataset.index;
        await this.toggleRecording(s, c);
      });
    }), this.shadowRoot.querySelectorAll('button[data-action="playback"]').forEach((d) => {
      d.addEventListener("click", (n) => {
        const s = n.target.closest("button");
        if (s.classList.contains("ready")) {
          const c = s.dataset.index;
          this.playRecording(c, s);
        }
      });
    }), this.shadowRoot.querySelectorAll('button[data-action="mp-guess"]').forEach((d) => {
      d.addEventListener("click", (n) => {
        const s = n.target.closest("button"), c = s.dataset.index, p = s.closest(".mp-container"), h = p.dataset.currentAnswer, g = this.shadowRoot.querySelector("#feedback-" + c);
        if (!h) {
          g.textContent = "Please listen to the audio first.", g.className = "feedback-msg";
          return;
        }
        const b = s.textContent.trim() === h.trim();
        if (p.querySelectorAll("button[data-action='mp-guess']").forEach((m) => m.disabled = !0), b) {
          s.classList.add("tj-btn-success"), s.classList.remove("tj-btn-secondary"), g.textContent = "Correct! 🎉", g.className = "feedback-msg correct";
          const m = s.closest(".tj-card");
          m && (m.classList.add("completed"), this.updateProgress());
        } else
          s.classList.add("tj-btn-error"), s.classList.remove("tj-btn-secondary"), g.textContent = "Incorrect.", g.className = "feedback-msg wrong", p.querySelectorAll("button[data-action='mp-guess']").forEach((m) => {
      ...
      d.addEventListener("click", (n) => {
        const s = n.target.dataset.index, c = this.shadowRoot.querySelector("#dropzone-" + s), p = this.shadowRoot.querySelector("#bank-" + s), h = this.shadowRoot.querySelector(
          "#feedback-" + s
        );
      ...
      d.addEventListener("click", (n) => {
        const s = n.target.dataset.index, c = this.shadowRoot.querySelector("#dropzone-" + s), p = this.shadowRoot.querySelector(
          "#feedback-" + s
        ...
        if (m === g) {
          ...
          const L = n.target.closest(".tj-card");
          L && (L.classList.add("completed"), this.updateProgress()), setTimeout(() => {
            p.textContent = "Activity Completed ✓", c.querySelectorAll(".scramble-word").forEach((S) => S.style.display = "none");
            const k = this.shadowRoot.querySelector("#bank-" + s);
  ...
  _generateReport() {
    const t = this.shadowRoot.getElementById("nickname-input"), e = this.shadowRoot.getElementById("number-input"), o = this.shadowRoot.getElementById("homeroom-input"), r = this.shadowRoot.getElementById("teacher-code-input"), a = t ? t.value.trim() : this.studentInfo.nickname, i = e ? e.value.trim() : this.studentInfo.number, l = o ? o.value.trim() : this.studentInfo.homeroom, u = r ? r.value.trim() : "";
    if (!a || !i) {
      alert("Please enter both nickname and student number.");
      return;
    }
    this.studentInfo = { nickname: a, number: i, homeroom: l, teacherCode: u };
    const y = this.shadowRoot.querySelectorAll(".tj-card[id^='act-']").length, d = this.shadowRoot.querySelectorAll(".tj-card.completed").length, n = Math.round(d / y * 100) || 0, s = (/* @__PURE__ */ new Date()).toLocaleString();
    let c = "🏆";
    n < 50 ? c = "💪" : n < 80 && (c = "⭐");
    const h = `
          <div class="rc-header">
    ...
    if (g && (g.style.display = "none"), b && (b.style.display = "block", b.innerHTML = h, this.recordings.size > 0)) {
      ...
      j.classList.add("recordings-list"), j.style.display = "flex", j.style.flexDirection = "column", j.style.gap = "8px", Array.from(this.recordings.keys()).sort((R, k) => R - k).forEach((R) => {
        ...
        if (k) {
          const z = k.querySelector(".lr-target-word");
          z && (S = z.textContent);
        }
        ...
        E.classList.add("recording-text"), E.style.fontSize = "0.9em", E.style.color = "var(--tj-text-main)", E.textContent = S, f.appendChild(w), f.appendChild(E), j.appendChild(f);
  ...
  async _submitScore() {
    ...
    const i = this.shadowRoot.querySelectorAll(".tj-card[id^='act-']").length, l = this.shadowRoot.querySelectorAll(".tj-card.completed").length, u = this.shadowRoot.getElementById("pronunciationTitle").textContent || "Pronunciation Practice", y = {
