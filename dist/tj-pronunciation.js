...
class H extends HTMLElement {
  ...
  render(e) {
    ...
    t.innerHTML = `<style>${_}</style><style>${O}</style>${V}`, this.shadowRoot.firstChild && (this.shadowRoot.innerHTML = ""), this.shadowRoot.appendChild(t.content.cloneNode(!0)), e.title && (this.shadowRoot.getElementById("pronunciationTitle").textContent = e.title);
    let o = "";
  ...
  playTTS(e, t) {
    return !this.synth || !this._shouldShowAudioControls() ? Promise.resolve() : new Promise((o, r) => {
      ...
      let h = !1;
      const g = () => {
        h || (h = !0, clearTimeout(s), t.classList.remove("playing"), this.isPlaying = !1, o());
      };
      a.onstart = () => {
        t.classList.add("playing"), this.isPlaying = !0;
      }, a.onend = () => g(), a.onerror = (c) => {
        h || (h = !0, clearTimeout(s), t.classList.remove("playing"), this.isPlaying = !1, c.error === "interrupted" || c.error === "canceled" ? o() : r(c));
      };
      const y = Math.max(3e3, e.length * 120), s = T() ? setTimeout(() => g(), y) : setTimeout(() => g(), y + 5e3);
      this.synth.speak(a);
    });
  }
  ...
  _updateVoiceList() {
    ...
    e.innerHTML = "", r.sort((i, l) => i.name.localeCompare(l.name)), r.forEach((i) => {
  ...
  playRecording(e, t) {
    ...
    r.oncanplaythrough = () => {
      ...
      console.error("Error loading recording for playback:", a), t.classList.remove("playing");
    }, r.src = o, r.load();
  }
}
