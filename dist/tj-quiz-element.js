...
}, A = `<div class="quiz-wrapper" translate="no">
...
class B extends HTMLElement {
  ...
  loadTemplate() {
    try {
      ...
      e.innerHTML = `<style>${E}</style>${A}`, this.shadowRoot.firstChild && (this.shadowRoot.innerHTML = ""), this.shadowRoot.appendChild(e.content.cloneNode(!0)), console.log("Inlined template applied successfully");
    } catch (e) {
...
`).trim();
    ...
    e.classList.remove("hidden"), t.innerHTML = "", this.vocabScore = 0, this.vocabUserChoices = {}, this.vocabSubmitted = !1, this.vocabularySections.forEach((o, s) => {
      ...
      if (d.className = "vocab-grid-header-cell", d.textContent = "Word", h.appendChild(d), a.forEach((l) => {
        ...
        this.shuffleArray(b), b.forEach((v, y) => {
          ...
          S.type = "radio", S.name = `vocab-${i}-${u}`, S.value = v, S.id = `vocab-${i}-${u}-${y}`, x.appendChild(S), w.appendChild(x);
  ...
  renderClozeInline(e, t, o) {
    ...
    r.forEach((g) => {
      const p = new RegExp(`\\*${g.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\*`, "gi");
      d = d.replace(p, () => {
        const m = `<input type="text" class="cloze-blank" data-answer="${g.toLowerCase()}" data-section-id="${i}" data-blank-index="${l}" autocomplete="off" spellcheck="false" inputmode="text" autocapitalize="none" autocorrect="off" title="Fill in the blank">`;
        return l++, m;
      });
    ...
    this.orderedSections.forEach((i) => {
      if (i.type === "audio") {
        ...
        d && d.appendChild(h), i.text.split(/\n\s*\n/).forEach((u) => {
          ...
          g.className = "passage-text", i.listening && g.classList.add("listening-hidden"), g.textContent = u.trim(), c.appendChild(g);
...
customElements.define("tj-quiz-element", B);
