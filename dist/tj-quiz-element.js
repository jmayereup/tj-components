import { c as I } from "./chunks/tj-config-DtZWnqEi.js";
...
class $ extends HTMLElement {
  ...
  _updateVoiceList() {
    ...
    if (!t) return;
    const o = "en-US", s = e.filter((i) => i.lang.split(/[-_]/)[0].toLowerCase() === o.split("-")[0]), a = this._getBestVoice(o);
    t.innerHTML = "", s.sort((i, r) => i.name.localeCompare(r.name)), s.forEach((i) => {
      const r = document.createElement("button");
      r.type = "button", r.classList.add("voice-option-btn"), this.selectedVoiceName === i.name && r.classList.add("active");
      let c = `<span>${i.name}</span>`;
      a && i.name === a.name && (c += '<span class="badge">Best</span>'), r.innerHTML = c, r.onclick = () => {
  ...
  parseContent() {
    ...
    if (t.length >= 1) {
      const r = t[0].trim().split(`
`).map((c) => c.trim()).filter((c) => c.length > 0);
      r.length > 0 && (this.title = r[0]);
    }
    let o = null, s = null;
...
`);
      ...
      const n = (c[0] || "").trim().toLowerCase(), d = c.slice(1).join(`
`);
      if (n.startsWith("vocab")) {
        const h = n.match(/vocab(?:-(\d+))?/), u = h && h[1] ? parseInt(h[1]) : null;
        this.parseVocabulary(d, u), this.orderedSections.push({ type: "vocab", data: { vocabCount: u } }), o = "vocab";
      } else if (n.startsWith("cloze")) {
        const h = n.match(/cloze(?:-(\d+))?/), u = h && h[1] ? parseInt(h[1]) : null;
        this.parseCloze(d, u), this.orderedSections.push({ type: "cloze", data: { clozeCount: u, text: d } }), o = "cloze";
      } else if (n.startsWith("instructions")) {
        const h = this.passages.length, { heading: u, body: g } = this.extractHeadingAndBody(d, `Instructions ${this.instructions.length + 1}`);
        this.instructions.push({ sectionId: h, heading: u, body: g }), this.passages.push({ text: g || u, sectionId: h, listening: !1, isInstruction: !0 }), this.orderedSections.push({ type: "instructions", sectionId: h, heading: u, body: g }), s = h, o = "instructions";
      } else if (n.startsWith("questions")) {
        const h = n.match(/questions(?:-(\d+))?/), u = h && h[1] ? parseInt(h[1]) : null, g = this.parseQuestions(d), f = o === "text" || o === "instructions" || o === "questions" && this.orderedSections.length > 0 && this.orderedSections[this.orderedSections.length - 1].tiedToPassage;
        s !== null ? (this.questionGroups.push({ sectionId: s, questions: g, maxQuestions: u }), this.orderedSections.push({ type: "questions", sectionId: s, questions: g, maxQuestions: u, tiedToPassage: f })) : (this.questionGroups.push({ sectionId: null, questions: g, maxQuestions: u }), this.orderedSections.push({ type: "questions", sectionId: null, questions: g, maxQuestions: u, tiedToPassage: !1 })), o = "questions";
      } else
        switch (n) {
          case "text":
          case "text-listening":
            const h = n === "text-listening", u = this.passages.length;
    ...
    const a = this.questionGroups.reduce((i, r) => i + (r.questions ? r.questions.length : 0), 0);
    console.log("Parsed:", {
      title: this.title,
  ...
  parseVocabulary(e, t = null) {
    if (!e) return;
    const o = e.split(/\r?\n/).map((c) => c.trim()).filter(Boolean), s = o.length > 0 ? o.slice() : [e.trim()], a = (c) => {
      const l = {};
      return c.forEach((n) => {
        const d = n.indexOf(":");
        if (d === -1) return;
        const h = n.slice(0, d).trim(), u = n.slice(d + 1).trim().replace(/,$/, "");
        h && u && (l[h] = u);
      }), l;
    };
    let i = a(s);
    if (Object.keys(i).length <= 1 && e.indexOf(",") !== -1) {
      const c = e.split(",").map((l) => l.trim()).filter(Boolean);
      i = a(c);
    }
    let r;
    if (t && Object.keys(i).length > t) {
      ...
      this.shuffleArray(c);
      const l = c.slice(0, t);
      r = Object.fromEntries(l);
    } else
      r = i;
    this.vocabularySections.push({
      vocabulary: r,
      sectionId: this.vocabularySections.length
    }), console.log("Vocabulary section parsed. Words in this section:", Object.keys(r).length, "Max words:", t);
  }
  parseAudio(e) {
    if (!e) return;
  ...
  parseCloze(e, t = null) {
    ...
    const o = e.match(/\*([^*]+)\*/g);
    let s = [];
    o && (s = o.map((a) => a.replace(/\*/g, "")), t && s.length > t && (this.shuffleArray(s), s = s.slice(0, t))), this.clozeSections.push({
      text: e,
      words: s,
...
`).map((i) => i.trim()).filter((i) => i.length > 0);
    ...
    for (const i of o)
      if (i.startsWith("Q:") || i.startsWith("Q."))
        ...
        const r = i.substring(2).trim(), c = r.includes("[correct]"), l = r.replace("[correct]", "").trim();
        s.o.push(l), c && (s.a = l);
      } else i.startsWith("E:") && s && (s.e = i.substring(2).trim());
    return s && a.push(s), console.log("Questions parsed. Total questions parsed:", a.length, "Max questions (deferred):", t), a;
  }
...
`);
    ...
    const a = [];
    for (const r of o)
      !s && r.trim().length > 0 ? s = r.trim() : a.push(r);
    s || (s = t);
    const i = a.join(`
`).trim();
  ...
  generateVocabMatching() {
    ...
    e.classList.remove("hidden"), t.innerHTML = "", this.vocabScore = 0, this.vocabUserChoices = {}, this.vocabSubmitted = !1, this.vocabularySections.forEach((o, s) => {
      const { vocabulary: a, sectionId: i } = o;
      if (!a) return;
      ...
      const r = Object.keys(a), c = Object.values(a);
      this.shuffleArray(c);
      const l = document.createElement("div");
      l.className = "vocab-grid-table";
      const n = document.createElement("div");
      n.className = "vocab-grid-header";
      const d = document.createElement("div");
      if (d.className = "vocab-grid-header-cell", d.textContent = "Word", n.appendChild(d), c.forEach((h) => {
        const u = document.createElement("div");
        u.className = "vocab-grid-header-cell", u.textContent = h, n.appendChild(u);
      }), l.appendChild(n), r.forEach((h, u) => {
        const g = document.createElement("div");
        g.className = "vocab-grid-row";
        const f = document.createElement("div");
        f.className = "vocab-grid-cell vocab-word-cell", f.textContent = h, g.appendChild(f);
        const p = a[h], w = c.filter((b) => b !== p);
        this.shuffleArray(w);
        const m = [p, ...w.slice(0, 3)];
        this.shuffleArray(m), m.forEach((b, S) => {
          const k = document.createElement("div");
          k.className = "vocab-grid-cell vocab-option-cell";
          const C = document.createElement("div");
          C.className = "vocab-radio-container";
          const x = document.createElement("input");
          x.type = "radio", x.name = `vocab-${i}-${u}`, x.value = b, x.id = `vocab-${i}-${u}-${S}`, C.appendChild(x), k.appendChild(C);
          const v = document.createElement("span");
          v.className = "vocab-def-label", v.textContent = b, k.appendChild(v), g.appendChild(k);
        }), l.appendChild(g);
  ...
  generateCloze() {
    ...
    o.innerHTML = "", this.clozeSections.forEach((s, a) => {
      const { text: i, words: r, sectionId: c } = s, l = document.createElement("div");
      if (l.className = "cloze-section-wrapper", this.clozeSections.length > 1) {
        const g = document.createElement("h4");
        g.className = "cloze-section-header", g.textContent = `Fill in the Blanks - Section ${a + 1}`, l.appendChild(g);
      }
      const n = document.createElement("div");
      n.className = "cloze-word-bank", n.innerHTML = `
      ...
      r.forEach((g) => {
        ...
        h = h.replace(f, () => {
          const p = `<input type="text" class="cloze-blank" data-answer="${g.toLowerCase()}" data-section-id="${c}" data-blank-index="${u}" autocomplete="off" spellcheck="false" title="Fill in the blank">`;
          return u++, p;
        });
      }), h = h.replace(/\*([^*]+)\*/g, "$1"), h = this.addLineBreaksToHtml(h), d.innerHTML = h, l.appendChild(d), a < this.clozeSections.length - 1 && (l.style.marginBottom = "2rem"), o.appendChild(l);
    });
  }
  // Render a single vocabulary section inline into the target container
  renderVocabInline(e, t, o) {
    const { vocabulary: s, sectionId: a } = e, r = this.vocabularySections.length > 1 ? `Vocabulary Set ${o + 1}` : "Vocabulary", { card: c, content: l } = this.createSectionCard(r, {
      cardClasses: ["vocab-card"]
    }), d = Object.keys(s).map((f, p) => ({
    ...
    h.className = "vocab-word-bank", h.innerHTML = `
            ...
            <div class="vocab-bank-items">
                ${d.map((f) => `<span class="vocab-bank-item">${f.letter}: ${f.word.toUpperCase()}</span>`).join("")}
            </div>
        `, l.appendChild(h);
    const u = document.createElement("div");
    ...
    this.shuffleArray(g), g.forEach((f) => {
      ...
      m.type = "text", m.className = "vocab-matching-input", m.maxLength = 1, m.dataset.sectionId = a, m.dataset.word = f.word, m.dataset.correctLetter = f.letter, m.autocomplete = "off", m.setAttribute("autocapitalize", "characters"), m.setAttribute("autocorrect", "off"), m.setAttribute("spellcheck", "false"), m.inputMode = "text", m.title = "Enter the letter for this definition", w.appendChild(m), p.appendChild(w);
      const b = document.createElement("div");
      b.className = "vocab-definition-text", b.textContent = f.definition, p.appendChild(b), u.appendChild(p);
    }), l.appendChild(u), t.appendChild(c);
  }
  // Render a single cloze section inline into the target container
  renderClozeInline(e, t, o) {
    const { text: s, words: a, sectionId: i } = e, r = this.clozeSections.length > 1 ? `Fill in the Blanks - Section ${o + 1}` : "Fill in the Blanks", { card: c, content: l } = this.createSectionCard(r, {
      cardClasses: ["cloze-card"]
    }), n = document.createElement("div");
    n.className = "cloze-word-bank", n.innerHTML = `
            <div class="cloze-bank-title">Word Bank</div>
            <div class="cloze-bank-words">
                ${a.map((g) => `<span class="cloze-bank-word">${g}</span>`).join("")}
            </div>
        `, l.appendChild(n);
    let d = s, h = 0;
    a.forEach((g) => {
      const f = new RegExp(`\\*${g.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\*`, "gi");
      d = d.replace(f, () => {
        const p = `<input type="text" class="cloze-blank" data-answer="${g.toLowerCase()}" data-section-id="${i}" data-blank-index="${h}" autocomplete="off" spellcheck="false" inputmode="text" autocapitalize="none" autocorrect="off" title="Fill in the blank">`;
        return h++, p;
      });
    }), d = d.replace(/\*([^*]+)\*/g, "$1"), d = this.addLineBreaksToHtml(d);
    const u = document.createElement("div");
    u.className = "cloze-text", u.innerHTML = d, l.appendChild(u), t.appendChild(c);
  }
  handleVocabAnswer(e) {
    const t = e.target;
    if (t.type === "text" && t.classList.contains("vocab-matching-input")) {
      ...
      t.value !== o && (t.value = o);
      const s = parseInt(t.dataset.sectionId), a = t.dataset.word, i = `${s}-${a}`;
  ...
  handleClozeAnswer(e) {
    ...
    const t = e.target, o = t.dataset.sectionId, s = t.dataset.blankIndex, a = t.value.trim().toLowerCase(), i = `${o}-${s}`;
    if (this.clozeAnswers[i] = a, this.checkAllClozeAnswered()) {
      const r = this.vocabularySections.length === 0 || Object.keys(this.vocabUserChoices).length === this.getTotalVocabWords(), c = this.totalQuestions === 0 || this.checkAllQuestionsAnswered();
      if (r && c) {
  ...
  handleShortAnswer(e) {
    ...
    const t = e.target, o = t.name, s = parseInt(o.substring(1)), a = t.value.trim();
    a ? (this.userQuestionAnswers[s] = a, t.dataset.answered = "true") : (delete this.userQuestionAnswers[s], delete t.dataset.answered);
  ...
  createSectionCard(e, t = {}) {
    const { descriptionHtml: o = "", cardClasses: s = [] } = t, a = document.createElement("div"), i = ["section-card", ...s].filter(Boolean);
    a.className = i.join(" ");
    const r = document.createElement("div");
    if (r.className = "section-card-header", r.textContent = e, a.appendChild(r), o) {
      const l = document.createElement("div");
      l.className = "section-card-description", l.innerHTML = o, a.appendChild(l);
    }
    const c = document.createElement("div");
    return c.className = "section-card-content", a.appendChild(c), { card: a, content: c };
  }
  showVocabScore() {
    this.vocabScore = 0, this.getTotalVocabWords(), this.vocabularySections.forEach((e) => {
      ...
      Object.keys(t).forEach((a) => {
        const i = `${o}-${a}`, r = this.vocabUserChoices[i], c = this.shadowRoot.querySelector(`.vocab-matching-input[data-section-id="${o}"][data-word="${a}"]`);
        if (!c) return;
        const l = c.dataset.correctLetter, n = c.closest(".vocab-matching-row");
        if (c.disabled = !0, !this.testMode) {
          let d = n.querySelector(".feedback-icon");
          d || (d = document.createElement("span"), d.className = "feedback-icon", n.appendChild(d)), r === l ? (n.classList.add("correct"), d.textContent = " ✅") : (n.classList.add("incorrect"), d.textContent = " ❌");
        }
        r === l && this.vocabScore++;
      });
  ...
  showClozeScore() {
    this.clozeScore = 0, this.clozeSections.reduce((t, o) => t + o.words.length, 0), this.shadowRoot.querySelectorAll(".cloze-blank").forEach((t) => {
      const o = this._normalizeText(t.dataset.answer), a = this._normalizeText(t.value) === o;
      a && this.clozeScore++, this.testMode || (a ? t.classList.add("correct") : t.classList.add("incorrect")), t.disabled = !0;
    }), this.clozeSubmitted = !0;
  }
  setupEventListeners() {
    const e = this.shadowRoot.getElementById("quizForm"), t = this.shadowRoot.getElementById("sendButton"), o = this.shadowRoot.getElementById("tryAgainButton"), s = this.shadowRoot.querySelector(".theme-toggle"), a = this.shadowRoot.getElementById("startQuizButton");
    ...
    }), e.addEventListener("submit", (n) => this.handleSubmit(n))), t && t.addEventListener("click", () => this.sendScore()), o && o.addEventListener("click", () => this.resetQuiz()), i && i.addEventListener("click", () => this.sendScore(!1, !0)), s && s.addEventListener("click", () => this.toggleTheme()), a && a.addEventListener("click", () => this.handleStartQuiz());
    const r = this.shadowRoot.getElementById("unlockTestButton"), c = this.shadowRoot.getElementById("lockTeacherCode"), l = this.shadowRoot.getElementById("lockErrorAlert");
    r && r.addEventListener("click", () => {
      const n = c ? c.value.trim() : "";
      if (n === this.code) {
        l && l.classList.add("hidden");
        const d = this.getTeacherCodeInput();
        d && (d.value = n), this.unlockQuizContent();
      } else
        l && (l.textContent = "❌ Invalid Teacher Code / รหัสผ่านไม่ถูกต้อง", l.classList.remove("hidden"));
    }), c && c.addEventListener("keydown", (n) => {
      n.key === "Enter" && (n.preventDefault(), r && r.click());
    }), this.getStudentInputs().forEach((n) => {
      ...
      const d = n.target.closest(".passage-audio-toggle");
      if (d) {
        const p = d.closest(".section-card"), m = (p ? Array.from(p.querySelectorAll(".passage-text")) : []).map((b) => b.textContent).join(`
`);
        this.handlePassageTTS(d, m);
        return;
      }
  ...
  handlePassageTTS(e, t) {
    if (e) {
      ...
      try {
        ...
        let s = window.speechSynthesis.getVoices().find((a) => a.name === this.selectedVoiceName);
  ...
  createQuestionBlock(e, t) {
    ...
    const i = e.e ? `<div class="explanation hidden" id="explanation-${o}">
        ...
        </div>` : "", r = document.createElement("div");
    return r.className = "question-block", r.innerHTML = `
  ...
  generateQuiz() {
    ...
    this.orderedSections.forEach((i) => {
      if (i.type === "audio") {
        ...
        const r = this.shadowRoot.querySelector(".quiz-header");
        if (r && !r.querySelector(".audio-toggle-container")) {
          const c = document.createElement("div");
          c.className = "audio-toggle-container", c.style.marginTop = "1rem", c.innerHTML = `
        ...
        const { card: r, content: c } = this.createSectionCard(i.heading || "Reading Passage", {
          cardClasses: ["passage-card"]
        }), l = document.createElement("div");
        l.className = "passage-wrapper";
        const n = document.createElement("button");
        n.type = "button", n.className = "passage-audio-toggle", n.title = "Play Passage Audio", n.innerHTML = `
                ...
                `;
        const d = r.querySelector(".section-card-header");
        d && d.appendChild(n), i.text.split(/\n\s*\n/).forEach((u) => {
          const g = document.createElement("p");
          g.className = "passage-text", i.listening && g.classList.add("listening-hidden"), g.textContent = u.trim(), l.appendChild(g);
        }), c.appendChild(l), t.appendChild(r);
      } else if (i.type === "instructions") {
        const r = i.heading || "Instructions", c = i.body ? this.formatTextWithLineBreaks(i.body) : "", { card: l } = this.createSectionCard(r, {
          descriptionHtml: c,
          cardClasses: ["instruction-card"]
        });
        t.appendChild(l);
      } else if (i.type === "vocab") {
        const r = this.vocabularySections[s++];
        r && this.renderVocabInline(r, t, s - 1);
      } else if (i.type === "cloze") {
        const r = this.clozeSections[a++];
        r && this.renderClozeInline(r, t, a - 1);
      } else if (i.type === "questions") {
        const { card: r, content: c } = this.createSectionCard("Comprehension Questions", {
          cardClasses: ["questions-card"]
        }), l = document.createElement("p");
        if (l.className = "reading-instructions instruction", l.textContent = "Read each question and select the best answer from the choices below.", c.appendChild(l), t.appendChild(r), i.questions && i.questions.length > 0) {
          const n = i.maxQuestions || null;
          let d = [...i.questions];
          n && d.length > n && (this.shuffleArray(d), d = d.slice(0, n)), d.forEach((h) => o.push({ question: h, container: c }));
        }
      }
    }), this.currentQuestions = o.map((i) => i.question), this.totalQuestions = this.currentQuestions.length, this.currentQuestions.forEach((i, r) => {
      const c = o[r];
      (c && c.container ? c.container : t).appendChild(this.createQuestionBlock(i, r));
    });
  }
  getStudentInputs() {
    return [
  ...
  saveCurrentStateToLocalStorage() {
    const e = this.getTotalVocabWords(), t = this.clozeSections.reduce((i, r) => i + r.words.length, 0), o = this.currentQuestions.filter((i) => i.o && i.o.length > 0).length, s = this.vocabScore + this.clozeScore + this.score, a = this.getTeacherCodeInput();
  ...
  restoreQuizState(e) {
    const t = this.shadowRoot.getElementById("nickname"), o = this.shadowRoot.getElementById("homeroom"), s = this.shadowRoot.getElementById("studentId"), a = this.getTeacherCodeInput();
    t && (t.value = e.nickname || ""), o && (o.value = e.homeroom || ""), s && (s.value = e.studentId || ""), a && (a.value = e.teacherCode || ""), this.vocabScore = e.vocabScore || 0, this.clozeScore = e.clozeScore || 0, this.score = e.score || 0, this.scoreSentToServer = e.scoreSentToServer || !1, this.userQuestionAnswers = e.userQuestionAnswers || {}, this.clozeAnswers = e.clozeAnswers || {}, this.vocabUserChoices = e.vocabUserChoices || {};
    for (let i = 0; i < this.totalQuestions; i++) {
      const r = `q${i}`, c = this.userQuestionAnswers[i];
      if (c !== void 0) {
        const l = this.currentQuestions[i];
        if (l.o && l.o.length > 0) {
          const n = this.shadowRoot.querySelector(`input[name="${r}"][value="${c}"]`);
          n && (n.checked = !0, n.dataset.answered = "true");
        } else {
          const n = this.shadowRoot.querySelector(`.short-answer-input[name="${r}"]`);
          n && (n.value = c, n.dataset.answered = "true", n.tagName.toLowerCase() === "textarea" && this.autoExpandTextarea(n));
        }
      }
    }
    Object.keys(this.vocabUserChoices).forEach((i) => {
      const r = this.vocabUserChoices[i], c = i.split("-"), l = c[0], n = c.slice(1).join("-"), d = this.shadowRoot.querySelector(`.vocab-matching-input[data-section-id="${l}"][data-word="${n}"]`);
      d && (d.value = r);
    }), Object.keys(this.clozeAnswers).forEach((i) => {
      const r = this.clozeAnswers[i], c = i.split("-"), l = c[0], n = c[1], d = this.shadowRoot.querySelector(`.cloze-blank[data-section-id="${l}"][data-blank-index="${n}"]`);
      d && (d.value = r);
  ...
  validateStudentInfoFields(e = {}) {
    ...
    let s = !0;
    return o.forEach((a) => {
      a.value.trim() === "" ? (s = !1, a.classList.add("invalid")) : a.classList.remove("invalid");
  ...
  showQuestionFeedback() {
    ...
    for (let e = 0; e < this.totalQuestions; e++) {
      ...
      if (!t.o || t.o.length === 0) {
        const r = this.shadowRoot.querySelector(`.short-answer-input[name="${o}"]`);
        r && (r.disabled = !0, r.classList.add("submitted"));
      } else
        this.shadowRoot.querySelectorAll(`input[name="${o}"]`).forEach((c) => {
          const l = c.closest(".option-label");
          if (c.disabled = !0, !this.testMode) {
            let n = l.querySelector(".feedback-icon");
            n || (n = document.createElement("span"), n.className = "feedback-icon", l.appendChild(n)), s === c.value && (s === t.a ? (l.classList.add("correct"), n.textContent = "✅") : (l.classList.add("incorrect"), n.textContent = "❌"));
          }
        }), s === t.a && this.score++;
  ...
  handleAnswer(e) {
    ...
    const i = this.vocabularySections.length === 0 || Object.keys(this.vocabUserChoices).length === this.getTotalVocabWords(), r = this.checkAllQuestionsAnswered(), c = this.checkAllClozeAnswered();
    i && r && c && (this.shadowRoot.getElementById("checkScoreButton").disabled = !1);
  }
  handleSubmit(e) {
  ...
  showFinalScore(e = !0) {
    ...
    const t = this.shadowRoot.getElementById("resultScore"), o = this.shadowRoot.getElementById("checkScoreContainer"), s = this.shadowRoot.getElementById("resultArea"), a = this.shadowRoot.getElementById("postScoreActions"), i = this.shadowRoot.getElementById("sendButton"), r = this.shadowRoot.getElementById("tryAgainButton"), c = this.shadowRoot.getElementById("studentInfoSection"), l = this.getTotalVocabWords(), n = this.clozeSections.reduce((v, y) => v + y.words.length, 0), d = this.currentQuestions.filter((v) => v.o && v.o.length > 0).length, h = l + n + d, u = this.vocabScore + this.clozeScore + this.score, g = this.shadowRoot.getElementById("nickname").value || "-", f = this.shadowRoot.getElementById("homeroom").value || "-", p = this.shadowRoot.getElementById("studentId").value || "-", m = (/* @__PURE__ */ new Date()).toLocaleString("en-GB", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" });
    if (c) {
    ...
    b && (this.scoreSentToServer ? b.classList.add("hidden") : b.classList.remove("hidden"));
    ...
    if (k && !this.testMode && (C = `
                ...
                <div class="written-answers-section">
                    ...
                    ${S.map((y, z) => {
      const A = this.currentQuestions.indexOf(y), E = this.userQuestionAnswers[A] || "-";
      return `
                    <div class="written-qa">
                        <div class="written-question">Q: ${y.q}</div>
                        <div class="written-answer">A: ${E}</div>
                    </div>
                `;
    }).join("")}
                </div>
      ...
      if (h > 0)
        if (this.testMode)
          ...
          const A = Math.round(u / h * 100);
          v = `
                        <div class="score-summary">
                            <div class="score-main-compact">${u} / ${h}</div>
                            <div class="score-percentage">${A}% Accuracy</div>
                        </div>
                    `;
        }
      let y = "";
      this.testMode || (l > 0 && (y += `
                        ...
                        <div class="score-section">
                            ...
                            <span class="score-value">${this.clozeScore}/${n}</span>
                        </div>`), d > 0 && (y += `
                        <div class="score-section">
    ...
    if (t.className = "", o && o.classList.add("hidden"), a && a.classList.remove("hidden"), s && s.classList.remove("hidden"), i && (i.disabled = !0, i.textContent = "Resend Score to Teacher", i.classList.add("hidden")), r && (r.disabled = !1, this.testMode ? r.classList.add("hidden") : r.classList.remove("hidden")), this.testMode) {
  ...
  async sendScore(e = !1, t = !1) {
    if (this.scoreSentToServer && !t || this.autoSubmissionInProgress)
      return;
    const o = this.shadowRoot.getElementById("validationMessage"), s = this.shadowRoot.getElementById("sendButton"), a = this.shadowRoot.getElementById("tryAgainButton"), i = this.shadowRoot.getElementById("retrySubmissionSection");
    ...
    this.vocabScore;
    const n = this.getTotalVocabWords(), d = this.clozeSections.reduce((p, w) => p + w.words.length, 0), h = this.currentQuestions.filter((p) => p.o && p.o.length > 0).length, u = n + d + h, g = this.vocabScore + this.clozeScore + this.score, f = {
      quizName: this.title,
    ...
    if (c !== this.code) {
      t ? o && (o.textContent = "❌ Invalid Teacher Code. Please try again.", o.className = "error") : (o && (o.innerHTML = `
                    ...
                    `, o.className = "warning"), i && i.classList.remove("hidden")), a && (a.disabled = !1), this.scoreSubmitted = !0, this.saveCurrentStateToLocalStorage(), this.autoSubmissionInProgress = !1;
      return;
    }
    if (!this.submissionUrl) {
      o && (o.textContent = "⚠️ No submission URL configured.", o.className = "error"), s && (s.textContent = "No Submission URL", s.disabled = !0, s.classList.remove("hidden")), a && (a.disabled = !1);
      return;
    }
    this.autoSubmissionInProgress = !0, s && (e ? s.classList.add("hidden") : (s.disabled = !0, s.textContent = "Sending...")), o && (o.innerHTML = e ? "<span>Submitting score to teacher...</span>" : "", o.className = ""), a && (a.disabled = !0);
    try {
      ...
      if (m && m.includes("application/json"))
        ...
        console.warn("Non-JSON response received:", b), w = { message: "Submission received (non-JSON response)" };
      }
      if (this.scoreSentToServer = !0, i && i.classList.add("hidden"), o) {
        const b = e ? "Score automatically submitted to your teacher" : w.message || "Submission successful!";
        o.innerHTML = `
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                    <span>${b}</span>
                `, o.className = "success";
      }
      s && (s.textContent = "Score Sent", s.disabled = !0, s.classList.add("hidden")), a && (a.disabled = !1), this.scoreSubmitted = !0, this.saveCurrentStateToLocalStorage();
    } catch (p) {
      console.error("Error:", p), o && (o.innerHTML = `
                ...
                `, o.className = "error"), s && (s.textContent = e ? "Send Score Again" : "Try Sending Again", s.disabled = !1, s.classList.remove("hidden")), a && (a.disabled = !1), this.saveCurrentStateToLocalStorage();
    } finally {
      this.autoSubmissionInProgress = !1;
    }
  }
  resetQuiz() {
    const e = this.shadowRoot.getElementById("quizForm"), t = this.shadowRoot.getElementById("resultArea"), o = this.shadowRoot.getElementById("postScoreActions"), s = this.shadowRoot.getElementById("checkScoreContainer"), a = this.shadowRoot.getElementById("validationMessage"), i = this.shadowRoot.getElementById("sendButton"), r = this.shadowRoot.getElementById("tryAgainButton"), c = this.getStudentInputs(), l = this.shadowRoot.getElementById("studentInfoSection");
    e.reset();
    const n = this.shadowRoot.getElementById("dynamicContent");
    if (n && n.classList.remove("hidden"), l) {
      l.style.display = "";
      const m = this.getStudentInputs(), b = this.getTeacherCodeInput();
      m.forEach((S) => {
        S && (S.disabled = !1);
      }), b && (b.disabled = !1);
    }
    t && t.classList.add("hidden"), o && o.classList.add("hidden"), s && s.classList.remove("hidden"), a && (a.textContent = "", a.className = ""), c.forEach((m) => {
    ...
    h && (h.value = ""), Array.from(this.shadowRoot.querySelectorAll('input[type="radio"]')).forEach((m) => {
      m.disabled = !1;
      try {
      ...
      m.tagName.toLowerCase() === "textarea" && this.autoExpandTextarea(m);
    }), Array.from(this.shadowRoot.querySelectorAll(".option-label")).forEach((m) => {
      m.classList.remove("correct", "incorrect");
      const b = m.querySelector(".feedback-icon");
      b && b.remove(), m.style.cursor = "";
    }), Array.from(this.shadowRoot.querySelectorAll(".explanation")).forEach((m) => m.classList.add("hidden")), i && (i.disabled = !1, i.textContent = "Resend Score to Teacher", i.classList.add("hidden")), r && (r.disabled = !1), this.stopAllAudio(), this.generateQuiz();
    const w = this.shadowRoot.getElementById("checkScoreButton");
    if (w && (w.disabled = !0), this.testMode) {
      const m = this.shadowRoot.getElementById("lockTeacherCode");
      m && (m.value = "");
  ...
  getWrittenAnswersString() {
    const t = this.currentQuestions.filter((s) => s.o && s.o.length === 0).map((s, a) => {
      const i = this.currentQuestions.indexOf(s), r = this.userQuestionAnswers[i] || "";
      return `Q: ${s.q}
A: ${r}`;
    }).join(`
