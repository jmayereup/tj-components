...
class R extends HTMLElement {
  constructor() {
    ...
    super(), this.attachShadow({ mode: "open" }), this.shadowRoot.innerHTML = `
      <style>${I}</style>
      ${A}
    `, this.data = [], this.studentInfo = { nickname: "", number: "", homeroom: "", teacherCode: "" }, this.submissionUrl = (r = z) == null ? void 0 : r.submissionUrl, this.isSubmitting = !1, this.score = 0, this.answeredCount = 0, this.isPlayingAll = !1, this.playbackIndex = 0, this.isPaused = !1, this.playbackUtterance = null, this.isAutoplay = !0, this.unscrambleData = [], this.currentUnscrambleIndex = 0, this.unscrambleScore = 0, this.userUnscrambledWords = [], this.memoryGameData = [], this.flippedCards = [], this.matchedPairsCount = 0, this.matchingGamesCompleted = 0, this.isCheckingMatch = !1, this.isSwapped = !1, this.selectedVoiceName = null, this.recordedBlobs = /* @__PURE__ */ new Map(), this.recordedSentences = /* @__PURE__ */ new Set(), this.completedIndices = /* @__PURE__ */ new Set(), this.unscrambleCompleted = !1, this.memoryCompleted = !1, this.unscrambleTotal = 0, this.memoryTotal = 0, this.isRecordingLine = null, this.mediaRecorder = null, this.recordingStartTime = 0, this.isPlayingRecording = null, this.shadowRoot.querySelector(".generate-btn").onclick = () => this.generateReport(), this.shadowRoot.querySelector("#play-pause-btn").onclick = () => this.toggleFullPlayback(), this.shadowRoot.querySelector("#stop-btn").onclick = () => this.stopFullPlayback(), this.shadowRoot.querySelector("#swap-btn").onclick = () => this.swapLanguages(), this.shadowRoot.querySelector("#voice-btn").onclick = () => this._showVoiceOverlay(), this.shadowRoot.querySelector(".close-voice-btn").onclick = () => this._hideVoiceOverlay(), this.shadowRoot.querySelector(".voice-overlay").onclick = (t) => {
      t.target.classList.contains("voice-overlay") && this._hideVoiceOverlay();
    };
    const e = this.shadowRoot.querySelector("#autoplay-checkbox");
    e.onchange = (t) => {
      this.isAutoplay = t.target.checked;
    };
  }
  connectedCallback() {
  ...
  loadData() {
    try {
      ...
      if (e) {
        const r = e.replace(/"((?:\\.|[^"\\])*)"/gs, (o, s) => '"' + s.replace(/\n/g, "\\n").replace(/\r/g, "\\r") + '"'), t = JSON.parse(r);
  ...
  _processParsedData(e) {
    this.data = (Array.isArray(e) ? e : [e]).map((r) => {
      const t = [...r.translationOptions], o = r.translationOptions[r.correctTranslationIndex];
      for (let a = t.length - 1; a > 0; a--) {
        const d = Math.floor(Math.random() * (a + 1));
        [t[a], t[d]] = [t[d], t[a]];
      }
      const s = r.original.split(/\s+/), i = r.highlightIndex, n = r.highlightIndexEnd !== void 0 ? r.highlightIndexEnd : i, c = s.slice(i, n + 1).join(" ").replace(/[.,!?;:]/g, "");
      return {
        ...r,
        highlightIndexEnd: n,
        shuffledOptions: t,
        newCorrectIndex: t.indexOf(o),
        originalWord: c,
        translationWord: o
      };
  ...
  displayAllLines() {
    ...
    const r = this.getAttribute("lang-original") || "en", t = this.getAttribute("lang-translation") || "th";
    this.data.forEach((o, s) => {
      const i = document.createElement("div");
      i.classList.add("card"), i.dataset.index = s;
      const n = document.createElement("div");
      n.classList.add("card-header");
      const c = document.createElement("div");
      c.classList.add("original-text"), o.original.split(" ").forEach((h, m) => {
        const l = document.createElement("span");
        l.textContent = h + " ", l.classList.add("tts-word"), !this.isSwapped && m >= o.highlightIndex && m <= o.highlightIndexEnd && l.classList.add("highlight"), l.onclick = (u) => {
          u.stopPropagation(), this.isPlayingAll && this.stopFullPlayback(), this._speak(h.replace(/[.,!?;:]/g, ""), r);
        }, c.appendChild(l);
      }), n.appendChild(c), i.appendChild(n), this.renderLineButtons(s, i);
      const a = document.createElement("div");
      a.classList.add("full-translation"), o.fullTranslation.split(" ").forEach((h, m) => {
        const l = document.createElement("span");
        l.textContent = h + " ", l.classList.add("tts-word"), this.isSwapped && m >= o.highlightIndex && m <= o.highlightIndexEnd && l.classList.add("highlight"), l.onclick = (u) => {
          u.stopPropagation(), this.isPlayingAll && this.stopFullPlayback(), this._speak(h.replace(/[.,!?;:]/g, ""), t);
        }, a.appendChild(l);
      });
      const d = document.createElement("div");
      d.classList.add("translation-options");
      const p = this.completedIndices.has(s);
      p && i.classList.add("completed", "answered"), o.shuffledOptions.forEach((h, m) => {
        const l = document.createElement("button");
        l.textContent = h, l.addEventListener("click", () => this.handleSelection(s, m, o.newCorrectIndex, l, i)), p && (l.disabled = !0, m !== o.newCorrectIndex ? l.style.opacity = "0.5" : l.classList.add("success")), d.appendChild(l);
      }), i.appendChild(a), i.appendChild(d), e.appendChild(i);
    }), this.updateProgress();
  }
  _getBestVoice(e) {
    return P(window.speechSynthesis, e);
  }
  _isMobile() {
  ...
  _updateVoiceList() {
    ...
    const e = window.speechSynthesis.getVoices(), r = this.shadowRoot.querySelector(".voice-list"), t = this.shadowRoot.querySelector("#voice-btn");
    if (!r || !t) return;
    const o = this.getAttribute("lang-original") || "en", s = o.split(/[-_]/)[0].toLowerCase();
    let i = e.filter((c) => c.lang.toLowerCase() === o.toLowerCase());
    if (i.length === 0 && (i = e.filter((c) => c.lang.split(/[-_]/)[0].toLowerCase() === s)), i.length === 0) {
      t.style.display = "none";
      return;
    }
    t.style.display = "flex", r.innerHTML = "";
    const n = this._getBestVoice(o);
    !this.selectedVoiceName && n && (this.selectedVoiceName = n.name), i.sort((c, a) => c.name.localeCompare(a.name)), i.forEach((c) => {
      const a = document.createElement("button");
      a.classList.add("voice-option-btn"), this.selectedVoiceName === c.name && a.classList.add("active");
      const d = document.createElement("span");
      if (d.textContent = c.name, a.appendChild(d), n && c.name === n.name) {
        const p = document.createElement("span");
        p.classList.add("badge"), p.textContent = "Best", a.appendChild(p);
      }
      a.onclick = () => {
        this.selectedVoiceName = c.name, this._updateVoiceList(), this._hideVoiceOverlay();
      }, r.appendChild(a);
    });
  }
  _showVoiceOverlay() {
  ...
  _speak(e, r, t = null) {
    ...
    let s = null;
    const i = this.getAttribute("lang-original") || "en", n = r.split(/[-_]/)[0].toLowerCase(), c = i.split(/[-_]/)[0].toLowerCase();
    return this.selectedVoiceName && n === c && (s = window.speechSynthesis.getVoices().find((d) => d.name === this.selectedVoiceName)), s || (s = this._getBestVoice(r)), s && (o.voice = s), o.lang = r, o.rate = 0.7, t && (o.onend = t), window.speechSynthesis.speak(o), o;
  }
  toggleFullPlayback() {
  ...
  async startRecording(e) {
    if (this.isRecordingLine === null)
      try {
        this.mediaRecorder = await T(
          (r) => {
            r.data.size > 0 && (this._audioChunks || (this._audioChunks = []), this._audioChunks.push(r.data));
          },
          (r) => {
            const t = new Blob(this._audioChunks, { type: r });
            Date.now() - this.recordingStartTime > 600 ? (this.recordedBlobs.set(e, t), this.recordedSentences.add(e)) : console.warn("Recording too short to be counted."), this.isRecordingLine = null, this._audioChunks = null, this.renderLineButtons(e);
          },
          1e3
  ...
  playRecordedAudio(e) {
    ...
    const t = URL.createObjectURL(r), o = new Audio(t);
    this.isPlayingRecording = e, this.renderLineButtons(e), o.play(), o.onended = () => {
      this.isPlayingRecording = null, this.renderLineButtons(e), URL.revokeObjectURL(t);
    };
  }
  renderLineButtons(e, r = null) {
    if (r || (r = this.shadowRoot.querySelector(`.card[data-index="${e}"]`)), !r) return;
    const t = r.querySelector(".card-header");
    let o = t.querySelector(".card-btn-group");
    o || (o = document.createElement("div"), o.classList.add("card-btn-group"), t.appendChild(o)), o.innerHTML = "";
  ...
  playLine(e, r = !1) {
    ...
    const t = this.data[e], o = this.getAttribute("lang-original") || "en";
    this.highlightCard(e), this.playbackUtterance = this._speak(t.original, o, () => {
      r && this.isPlayingAll && !this.isPaused && this.playLine(e + 1, !0);
    });
  }
  highlightCard(e) {
    this.clearPlaybackHighlights();
    const t = this.shadowRoot.querySelectorAll(".card")[e];
    t && (t.classList.add("playing"), t.scrollIntoView({ behavior: "smooth", block: "center" }));
  }
  ...
  _shouldShowAudioControls() {
    return M(window.speechSynthesis);
  }
  ...
  checkBrowserSupport() {
    if (!this._shouldShowAudioControls()) {
      ...
      if (e) {
        ...
        const r = this._getAndroidIntentLink(), t = this.shadowRoot.querySelector(".browser-action-btn");
        r ? (t.href = r, t.textContent = "Open in Chrome") : (t.onclick = (o) => {
          (!t.href || t.href === "javascript:void(0)") && (o.preventDefault(), alert("Please open this page in Safari or Chrome for the best experience with audio features."));
        }, t.textContent = "Use Safari / Chrome");
      }
    }
  }
  handleSelection(e, r, t, o, s) {
    if (s.classList.contains("answered")) return;
    if (r === t) {
      s.classList.add("answered"), this.answeredCount++, this.completedIndices.add(e), s.dataset.hadError || this.score++, o.classList.add("success"), s.classList.add("completed"), s.classList.remove("failed"), s.querySelectorAll(".translation-options button").forEach((a) => {
        a.disabled = !0, a !== o && (a.style.opacity = "0.5");
      }), this.updateProgress();
      ...
      o.classList.add("error"), o.disabled = !0, s.classList.add("failed"), s.dataset.hadError = "true";
  }
  updateProgress() {
  ...
  startUnscrambleActivity(e = !0) {
    ...
    const t = r.filter((i) => !this.unscrambleUsedSentences.has(i));
    let o = [];
    if (t.length >= 5)
      o = t.sort(() => 0.5 - Math.random()).slice(0, 5);
    else {
      o = [...t];
      const i = 5 - o.length, n = r.filter((c) => !t.includes(c)).sort(() => 0.5 - Math.random());
      o = o.concat(n.slice(0, i));
    }
    if (o.forEach((i) => this.unscrambleUsedSentences.add(i)), this.unscrambleUsedSentences.size >= this.data.length && (this.unscrambleUsedSentences.clear(), o.forEach((i) => this.unscrambleUsedSentences.add(i))), this.unscrambleData = o.map((i) => {
      const n = this.data[i], c = n.original.split(/\s+/).filter((d) => d.length > 0), a = [...c].sort(() => 0.5 - Math.random());
      return {
        ...n,
        correctWords: c,
        shuffledWords: a
  ...
  renderUnscrambleChallenge(e = !0) {
    ...
    r.innerHTML = "";
    const t = this.unscrambleData[this.currentUnscrambleIndex], o = document.createElement("div");
    o.classList.add("card", "unscramble-card", "playing");
    const s = document.createElement("h3");
    s.innerHTML = `Unscramble the Sentence <span style="font-size: 0.8em; color: #64748b; font-weight: normal; margin-left: 0.5em; white-space: nowrap;">(${this.currentUnscrambleIndex + 1} / ${this.unscrambleTotal})</span>`, o.appendChild(s);
    ...
    n.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>', n.classList.add("voice-btn"), n.style.margin = "0 auto 1em auto", n.onclick = () => {
      this._speak(t.original, i);
    }, o.appendChild(n);
    const c = document.createElement("div");
    c.classList.add("full-translation"), c.style.fontSize = "1.2em", c.textContent = t.fullTranslation, c.setAttribute("lang", this.getAttribute("lang-translation") || "th"), o.appendChild(c);
    const a = document.createElement("div");
    a.classList.add("unscramble-result"), a.setAttribute("lang", i), o.appendChild(a);
    const d = document.createElement("div");
    d.classList.add("unscramble-pool"), d.setAttribute("lang", i), o.appendChild(d);
    const p = () => {
      ...
      }), d.innerHTML = "", t.shuffledWords.forEach((g, v) => {
        const y = this.userUnscrambledWords.filter((f) => f === g).length, b = t.shuffledWords.filter((f) => f === g).length;
        if (y < b) {
    ...
    const l = document.createElement("button");
    l.textContent = "Check", l.classList.add("finish-btn"), l.style.padding = "0.8em 2em", l.onclick = () => {
      this.userUnscrambledWords.join(" ") === t.correctWords.join(" ") ? (this.unscrambleScore++, l.textContent = "Correct! Next", l.classList.add("success"), l.onclick = () => {
        this.currentUnscrambleIndex++, this.userUnscrambledWords = [], this.currentUnscrambleIndex < this.unscrambleData.length ? (this.renderUnscrambleChallenge(), this.updateProgress()) : this.renderUnscrambleCompletion();
      }) : (l.classList.add("error"), setTimeout(() => l.classList.remove("error"), 500));
    };
    const u = document.createElement("button");
    u.textContent = "Reset", u.onclick = () => {
      this.userUnscrambledWords = [], p();
    }, h.appendChild(u), h.appendChild(m), h.appendChild(l), o.appendChild(h), r.appendChild(o), e && o.scrollIntoView({ behavior: "smooth", block: "center" });
  }
  startMemoryGame(e = !0) {
    ...
    for (; t.length < 6 && o.size < r.length; ) {
      const i = Math.floor(Math.random() * r.length);
      o.has(i) || (t.push(r[i]), o.add(i));
    }
    const s = [];
    t.forEach((i, n) => {
  ...
  renderMemoryGameUI() {
    ...
    t.classList.add("memory-grid"), this.memoryGameData.forEach((n, c) => {
      const a = document.createElement("div");
      ...
      `, a.onclick = () => this.handleMemoryCardFlip(a, n), t.appendChild(a);
    }), e.appendChild(t);
    const o = document.createElement("div");
    o.classList.add("memory-game-actions");
    const s = document.createElement("button");
    s.textContent = "Play Again (New Words)", s.onclick = () => this.startMemoryGame(), o.appendChild(s), e.appendChild(o);
  ...
  handleMemoryCardFlip(e, r) {
    if (!(this.isCheckingMatch || e.classList.contains("flipped") || e.classList.contains("matched")) && (this._speak(r.text, r.lang), e.classList.add("flipped"), this.flippedCards.push({ element: e, data: r }), this.flippedCards.length === 2)) {
      ...
      const [t, o] = this.flippedCards;
      t.data.id === o.data.id ? setTimeout(() => {
        t.element.classList.add("matched"), o.element.classList.add("matched"), this.matchedPairsCount++, this.flippedCards = [], this.isCheckingMatch = !1, this.updateProgress(), this.matchedPairsCount === this.memoryGameData.length / 2 && (this.memoryCompleted = !0);
      }, 600) : setTimeout(() => {
        t.element.classList.remove("flipped"), o.element.classList.remove("flipped"), this.flippedCards = [], this.isCheckingMatch = !1;
      }, 1200);
    }
  }
  showFinalForm() {
    ...
    const t = this.shadowRoot.querySelector("#nickname"), o = this.shadowRoot.querySelector("#student-number");
    this.studentInfo.nickname ? (t.value = this.studentInfo.nickname, t.disabled = !0, o.value = this.studentInfo.number, o.disabled = !0, this.generateReport()) : (this.shadowRoot.querySelector(".initial-form").style.display = "block", this.shadowRoot.querySelector(".report-area").innerHTML = "");
  }
  generateReport() {
    const e = this.shadowRoot.querySelector("#nickname").value.trim(), r = this.shadowRoot.querySelector("#student-number").value.trim(), t = this.shadowRoot.querySelector("#homeroom").value.trim();
    if (!e || !r) {
      alert("Please enter both nickname and student number.");
      return;
    }
    this.studentInfo = { ...this.studentInfo, nickname: e, number: r, homeroom: t };
    const o = this.getAttribute("story-title") || "Story Practice", s = (/* @__PURE__ */ new Date()).toLocaleString(), i = this.data.length > 0 ? this.score / this.data.length : 0;
    this.data.length > 0 && this.recordedSentences.size / this.data.length;
    const n = this.unscrambleData.length || this.unscrambleTotal, c = n > 0 ? this.unscrambleScore / n : 0, a = this.memoryGameData.length / 2 || this.memoryTotal, d = a > 0 ? this.matchedPairsCount / a : 0;
    let p = 85, h = 10, m = 5;
    this.unscrambleData.length === 0 && (p += h, h = 0);
    const l = i * p + c * h + d * m, u = this.shadowRoot.querySelector(".report-area"), g = l >= 80 ? "🏆" : l >= 50 ? "⭐" : "💪", v = l >= 80 ? "Excellent!" : l >= 50 ? "Good effort!" : "Keep practicing!";
    if (u.innerHTML = `
      <div class="rc-header">
          <div class="rc-icon">📄</div>
          <div class="rc-title">${o}</div>
          <div class="rc-subtitle">Report Card</div>
      </div>
      <div class="rc-student">
          <span class="rc-label">Student</span>
          <span class="rc-value">${e} <span class="rc-number">(${r}) ${t ? `- ${t}` : ""}</span></span>
      </div>
      <div class="rc-score-row">
          <div class="rc-score-circle">
              <div class="rc-score-val">${Math.round(l)}%</div>
              <div class="rc-score-pct">Overall</div>
          </div>
          <div class="rc-score-label">${g} ${v}</div>
      </div>
      <div class="rc-bar-track" style="margin: 0 0 16px 0;"><div class="rc-bar-fill" style="width:${l}%"></div></div>
    ...
    `, this.recordedBlobs.size > 0) {
      ...
      b.classList.add("recordings-section");
      const f = document.createElement("h4");
      f.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="#2563eb"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg> Student Recordings', b.appendChild(f);
      const C = document.createElement("div");
    ...
    this.shadowRoot.querySelector(".initial-form").style.display = "none", u.querySelector(".return-btn").onclick = () => {
      this.shadowRoot.querySelector(".form-overlay").style.display = "none";
      const b = this.shadowRoot.querySelector(".sticky-bar");
      b && (b.style.display = "flex");
    };
    const y = u.querySelector("#submit-score-btn");
    y && (y.onclick = () => this._submitScore()), u.querySelector(".reset-all-btn").onclick = () => {
      var b;
      confirm("Are you sure you want to reset all progress? This will delete all your scores and recordings.") && (this.completedIndices.clear(), this.recordedBlobs.clear(), this.recordedSentences.clear(), this.unscrambleScore = 0, this.matchedPairsCount = 0, this.unscrambleCompleted = !1, this.memoryCompleted = !1, this.score = 0, this.answeredCount = 0, (b = this.unscrambleUsedSentences) == null || b.clear(), this.shadowRoot.querySelector(".form-overlay").style.display = "none", this.shadowRoot.querySelector(".report-area").innerHTML = "", this.shadowRoot.querySelector(".initial-form").style.display = "block", this.loadData());
    };
  }
  async _submitScore() {
    ...
    const t = this.shadowRoot.getElementById("submit-score-btn"), o = t.textContent;
    this.isSubmitting = !0, t.textContent = "Submitting...", t.disabled = !0;
    const s = this.getAttribute("story-title") || "Story Practice", i = this.data.length > 0 ? this.score / this.data.length : 0, n = this.unscrambleData.length || this.unscrambleTotal, c = n > 0 ? this.unscrambleScore / n : 0, a = this.memoryGameData.length / 2 || this.memoryTotal, d = a > 0 ? this.matchedPairsCount / a : 0;
    let p = 85, h = 10, m = 5;
    this.unscrambleData.length === 0 && (p += h, h = 0);
    const l = i * p + c * h + d * m, u = {
    ...
    try {
      ...
      }), alert("Score successfully submitted!"), t.textContent = "Submitted ✓", t.style.background = "var(--tj-text-muted)";
    } catch (g) {
      console.error("Error submitting score:", g), alert("There was an error submitting your score. Please try again."), t.textContent = o, t.disabled = !1, this.isSubmitting = !1;
    }
  }
  swapLanguages() {
    const e = this.getAttribute("lang-original") || "en", r = this.getAttribute("lang-translation") || "th";
    this.setAttribute("lang-original", r), this.setAttribute("lang-translation", e), this.isSwapped = !this.isSwapped, this.selectedVoiceName = null, this._updateVoiceList(), this.data = this.data.map((t) => {
      const o = t.fullTranslation, s = t.original, i = t.translationWord, n = t.originalWord;
      return {
        ...t,
        original: o,
        fullTranslation: s,
...
customElements.get("tj-reader") || customElements.define("tj-reader", R);
customElements.get("lbl-reader") || customElements.define("lbl-reader", class extends R {
});
