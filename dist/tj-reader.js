...
class L extends HTMLElement {
  ...
  loadData() {
    try {
      ...
      if (e) {
        const o = e.replace(/"((?:\\.|[^"\\])*)"/gs, (t, n) => '"' + n.replace(/\n/g, "\\n").replace(/\r/g, "\\r") + '"'), i = JSON.parse(o);
        this._processParsedData(i);
      }
    } catch (e) {
  ...
  _processParsedData(e) {
    this.data = (Array.isArray(e) ? e : [e]).map((o) => {
      const i = [...o.translationOptions], t = o.translationOptions[o.correctTranslationIndex];
      for (let a = i.length - 1; a > 0; a--) {
        const d = Math.floor(Math.random() * (a + 1));
        [i[a], i[d]] = [i[d], i[a]];
      }
      const n = o.original.split(/\s+/), s = o.highlightIndex, r = o.highlightIndexEnd !== void 0 ? o.highlightIndexEnd : s, l = n.slice(s, r + 1).join(" ").replace(/[.,!?;:]/g, "");
      return {
        ...o,
        highlightIndexEnd: r,
        shuffledOptions: i,
        newCorrectIndex: i.indexOf(t),
        originalWord: l,
        translationWord: t
      };
  ...
  displayAllLines() {
    ...
    const o = this.getAttribute("lang-original") || "en", i = this.getAttribute("lang-translation") || "th";
    this.data.forEach((t, n) => {
      const s = document.createElement("div");
      s.classList.add("card"), s.dataset.index = n;
      const r = document.createElement("div");
      r.classList.add("card-header");
      ...
      }), r.appendChild(l), s.appendChild(r), this.renderLineButtons(n, s);
      ...
      const p = this.completedIndices.has(n);
      p && s.classList.add("completed", "answered"), t.shuffledOptions.forEach((h, m) => {
        const c = document.createElement("button");
        c.textContent = h, c.addEventListener("click", () => this.handleSelection(n, m, t.newCorrectIndex, c, s)), p && (c.disabled = !0, m !== t.newCorrectIndex ? c.style.opacity = "0.5" : c.classList.add("success")), d.appendChild(c);
      }), s.appendChild(a), s.appendChild(d), e.appendChild(s);
    }), this.updateProgress();
  }
  _getBestVoice(e) {
    return R(window.speechSynthesis, e);
  }
  _isMobile() {
  ...
  _updateVoiceList() {
    ...
    const e = window.speechSynthesis.getVoices(), o = this.shadowRoot.querySelector(".voice-list"), i = this.shadowRoot.querySelector("#voice-btn");
    if (!o || !i) return;
    const t = this.getAttribute("lang-original") || "en", n = t.split(/[-_]/)[0].toLowerCase();
    let s = e.filter((l) => l.lang.toLowerCase() === t.toLowerCase());
    if (s.length === 0 && (s = e.filter((l) => l.lang.split(/[-_]/)[0].toLowerCase() === n)), s.length === 0) {
      i.style.display = "none";
      return;
    }
    i.style.display = "flex", o.innerHTML = "";
    const r = this._getBestVoice(t);
    !this.selectedVoiceName && r && (this.selectedVoiceName = r.name), s.sort((l, a) => l.name.localeCompare(a.name)), s.forEach((l) => {
      const a = document.createElement("button");
      a.classList.add("voice-option-btn"), this.selectedVoiceName === l.name && a.classList.add("active");
      const d = document.createElement("span");
      if (d.textContent = l.name, a.appendChild(d), r && l.name === r.name) {
  ...
  _speak(e, o, i = null) {
    ...
    let n = null;
    const s = this.getAttribute("lang-original") || "en", r = o.split(/[-_]/)[0].toLowerCase(), l = s.split(/[-_]/)[0].toLowerCase();
    return this.selectedVoiceName && r === l && (n = window.speechSynthesis.getVoices().find((d) => d.name === this.selectedVoiceName)), n || (n = this._getBestVoice(o)), n && (t.voice = n), t.lang = o, t.rate = 0.7, i && (t.onend = i), window.speechSynthesis.speak(t), t;
  }
  toggleFullPlayback() {
    ...
    if (this.isRecordingLine === null)
      try {
        this.mediaRecorder = await P(
          (o) => {
            o.data.size > 0 && (this._audioChunks || (this._audioChunks = []), this._audioChunks.push(o.data));
          },
          (o) => {
            const i = new Blob(this._audioChunks, { type: o });
            Date.now() - this.recordingStartTime > 600 ? (this.recordedBlobs.set(e, i), this.recordedSentences.add(e)) : console.warn("Recording too short to be counted."), this.isRecordingLine = null, this._audioChunks = null, this.renderLineButtons(e);
          },
          1e3
        ), this.recordingStartTime = Date.now(), this.isRecordingLine = e, this.renderLineButtons(e);
      } catch (o) {
        console.error("Error starting recording:", o), alert("Could not access microphone. Please check permissions.");
      }
  }
  stopRecording() {
  ...
  playRecordedAudio(e) {
    const o = this.recordedBlobs.get(e);
    if (!o) return;
    this.isPlayingRecording;
    const i = URL.createObjectURL(o), t = new Audio(i);
    this.isPlayingRecording = e, this.renderLineButtons(e), t.play(), t.onended = () => {
      this.isPlayingRecording = null, this.renderLineButtons(e), URL.revokeObjectURL(i);
    };
  }
  renderLineButtons(e, o = null) {
    if (o || (o = this.shadowRoot.querySelector(`.card[data-index="${e}"]`)), !o) return;
    const i = o.querySelector(".card-header");
    let t = i.querySelector(".card-btn-group");
    t || (t = document.createElement("div"), t.classList.add("card-btn-group"), i.appendChild(t)), t.innerHTML = "";
    const n = document.createElement("button");
    n.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>', n.classList.add("voice-btn"), n.title = "Play TTS", n.onclick = () => {
      this.isPlayingAll && this.stopFullPlayback(), this.playLine(e, !1);
    }, t.appendChild(n);
    const s = document.createElement("button");
    if (s.classList.add("record-btn"), this.isRecordingLine === e ? (s.classList.add("recording"), s.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><rect x="6" y="6" width="12" height="12"/></svg>', s.onclick = () => this.stopRecording(), s.title = "Stop Recording") : (s.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>', s.onclick = () => this.startRecording(e), s.title = "Record Voice"), t.appendChild(s), this.recordedBlobs.has(e) && this.isRecordingLine !== e) {
      const r = document.createElement("button");
      this.isPlayingRecording === e ? (r.classList.add("play-recorded-btn", "playing"), r.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z"/></svg>') : (r.classList.add("play-recorded-btn"), r.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M10 16.5l6-4.5-6-4.5v9zM12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>'), r.title = "Play Recording", r.onclick = () => this.playRecordedAudio(e), t.appendChild(r);
    }
  }
  playLine(e, o = !1) {
    if (e >= this.data.length) {
      o && this.stopFullPlayback();
      return;
    }
    this.playbackIndex = e;
    const i = this.data[e], t = this.getAttribute("lang-original") || "en";
    this.highlightCard(e), this.playbackUtterance = this._speak(i.original, t, () => {
      o && this.isPlayingAll && !this.isPaused && this.playLine(e + 1, !0);
    });
  }
  highlightCard(e) {
    this.clearPlaybackHighlights();
    const i = this.shadowRoot.querySelectorAll(".card")[e];
    i && (i.classList.add("playing"), i.scrollIntoView({ behavior: "smooth", block: "center" }));
  }
  ...
  updatePlaybackUI() {
    ...
    const o = this.shadowRoot.querySelector("#stop-btn");
    o && (o.style.display = this.isPlayingAll ? "flex" : "none");
  }
  ...
  checkBrowserSupport() {
    if (!this._shouldShowAudioControls()) {
      ...
      if (e) {
        ...
        const o = this._getAndroidIntentLink(), i = this.shadowRoot.querySelector(".browser-action-btn");
        o ? (i.href = o, i.textContent = "Open in Chrome") : (i.onclick = (t) => {
          (!i.href || i.href === "javascript:void(0)") && (t.preventDefault(), alert("Please open this page in Safari or Chrome for the best experience with audio features."));
        }, i.textContent = "Use Safari / Chrome");
      }
    }
  }
  handleSelection(e, o, i, t, n) {
    if (n.classList.contains("answered")) return;
    if (o === i) {
      n.classList.add("answered"), this.answeredCount++, this.completedIndices.add(e), n.dataset.hadError || this.score++, t.classList.add("success"), n.classList.add("completed"), n.classList.remove("failed"), n.querySelectorAll(".translation-options button").forEach((a) => {
      ...
      t.classList.add("error"), t.disabled = !0, n.classList.add("failed"), n.dataset.hadError = "true";
  }
  updateProgress() {
  ...
  startUnscrambleActivity(e = !0) {
    ...
    let o = String.keys ? Object.keys(this.data) : Array.from(this.data.keys());
    o = o.map((s) => parseInt(s));
    const i = o.filter((s) => !this.unscrambleUsedSentences.has(s));
    let t = [];
    if (i.length >= 5)
      t = i.sort(() => 0.5 - Math.random()).slice(0, 5);
    else {
      t = [...i];
      const s = 5 - t.length, r = o.filter((l) => !i.includes(l)).sort(() => 0.5 - Math.random());
      t = t.concat(r.slice(0, s));
    }
    if (t.forEach((s) => this.unscrambleUsedSentences.add(s)), this.unscrambleUsedSentences.size >= this.data.length && (this.unscrambleUsedSentences.clear(), t.forEach((s) => this.unscrambleUsedSentences.add(s))), this.unscrambleData = t.map((s) => {
      const r = this.data[s], l = r.original.split(/\s+/).filter((d) => d.length > 0), a = [...l].sort(() => 0.5 - Math.random());
      return {
        ...r,
        correctWords: l,
        shuffledWords: a
  ...
  renderUnscrambleChallenge(e = !0) {
    const o = this.shadowRoot.querySelector(".scramble-container");
    o.innerHTML = "";
    const i = this.unscrambleData[this.currentUnscrambleIndex], t = document.createElement("div");
    t.classList.add("card", "unscramble-card", "playing");
    const n = document.createElement("h3");
    n.innerHTML = `Unscramble the Sentence <span style="font-size: 0.8em; color: #64748b; font-weight: normal; margin-left: 0.5em; white-space: nowrap;">(${this.currentUnscrambleIndex + 1} / ${this.unscrambleTotal})</span>`, t.appendChild(n);
    const s = this.getAttribute("lang-original") || "en", r = document.createElement("button");
    r.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>', r.classList.add("voice-btn"), r.style.margin = "0 auto 1em auto", r.onclick = () => {
      this._speak(i.original, s);
    }, t.appendChild(r);
    const l = document.createElement("div");
    l.classList.add("full-translation"), l.style.fontSize = "1.2em", l.textContent = i.fullTranslation, l.setAttribute("lang", this.getAttribute("lang-translation") || "th"), t.appendChild(l);
    const a = document.createElement("div");
    a.classList.add("unscramble-result"), a.setAttribute("lang", s), t.appendChild(a);
    const d = document.createElement("div");
    d.classList.add("unscramble-pool"), d.setAttribute("lang", s), t.appendChild(d);
    const p = () => {
      ...
      }), d.innerHTML = "", i.shuffledWords.forEach((b, u) => {
        const f = this.userUnscrambledWords.filter((y) => y === b).length, w = i.shuffledWords.filter((y) => y === b).length;
        if (f < w) {
    ...
    c.textContent = "Check", c.classList.add("finish-btn"), c.style.padding = "0.8em 2em", c.onclick = () => {
      this.userUnscrambledWords.join(" ") === i.correctWords.join(" ") ? (this.unscrambleScore++, c.textContent = "Correct! Next", c.classList.add("success"), c.onclick = () => {
        this.currentUnscrambleIndex++, this.userUnscrambledWords = [], this.currentUnscrambleIndex < this.unscrambleData.length ? (this.renderUnscrambleChallenge(), this.updateProgress()) : this.renderUnscrambleCompletion();
    ...
    }, h.appendChild(g), h.appendChild(m), h.appendChild(c), t.appendChild(h), o.appendChild(t), e && t.scrollIntoView({ behavior: "smooth", block: "center" });
  }
  startMemoryGame(e = !0) {
    ...
    const o = this.data.map((s) => ({
      original: s.originalWord,
      translation: s.translationWord
    })), i = [], t = /* @__PURE__ */ new Set();
    for (; i.length < 6 && t.size < o.length; ) {
      const s = Math.floor(Math.random() * o.length);
      t.has(s) || (i.push(o[s]), t.add(s));
    }
    const n = [];
    i.forEach((s, r) => {
      n.push({ id: r, type: "original", text: s.original, lang: this.getAttribute("lang-original") || "en" }), n.push({ id: r, type: "translation", text: s.translation, lang: this.getAttribute("lang-translation") || "th" });
    }), this.memoryGameData = n.sort(() => 0.5 - Math.random()), this.memoryTotal = this.memoryGameData.length / 2, this.renderMemoryGameUI(), this.updateProgress();
  }
  ...
  renderMemoryGameUI() {
    ...
    o.classList.add("memory-game-header"), o.innerHTML = `
      <h3>Memory Matching Game</h3>
      <p>Match the word with its translation!</p>
    `, e.appendChild(o);
    const i = document.createElement("div");
    i.classList.add("memory-grid"), this.memoryGameData.forEach((r, l) => {
      const a = document.createElement("div");
      a.classList.add("memory-card"), a.dataset.index = l, a.innerHTML = `
        <div class="memory-card-inner">
          <div class="memory-card-front">?</div>
          <div class="memory-card-back" lang="${r.lang}">${r.text}</div>
        </div>
      `, a.onclick = () => this.handleMemoryCardFlip(a, r), i.appendChild(a);
    }), e.appendChild(i);
    const t = document.createElement("div");
    t.classList.add("memory-game-actions");
    const n = document.createElement("button");
    n.textContent = "Play Again (New Words)", n.onclick = () => this.startMemoryGame(), t.appendChild(n), e.appendChild(t);
    const s = this.shadowRoot.querySelector(".finish-container");
    s.style.display = "block", s.querySelector(".finish-btn").onclick = () => this.showFinalForm();
  }
  handleMemoryCardFlip(e, o) {
    if (!(this.isCheckingMatch || e.classList.contains("flipped") || e.classList.contains("matched")) && (this._speak(o.text, o.lang), e.classList.add("flipped"), this.flippedCards.push({ element: e, data: o }), this.flippedCards.length === 2)) {
      this.isCheckingMatch = !0;
      const [i, t] = this.flippedCards;
      i.data.id === t.data.id ? setTimeout(() => {
        i.element.classList.add("matched"), t.element.classList.add("matched"), this.matchedPairsCount++, this.flippedCards = [], this.isCheckingMatch = !1, this.updateProgress(), this.matchedPairsCount === this.memoryGameData.length / 2 && (this.memoryCompleted = !0);
      }, 600) : setTimeout(() => {
        i.element.classList.remove("flipped"), t.element.classList.remove("flipped"), this.flippedCards = [], this.isCheckingMatch = !1;
      }, 1200);
    }
  }
  showFinalForm() {
    ...
    e && (e.style.display = "none");
    const o = this.shadowRoot.querySelector(".form-overlay");
    o.style.display = "flex";
    const i = this.shadowRoot.querySelector("#nickname"), t = this.shadowRoot.querySelector("#student-number");
    this.studentInfo.nickname ? (i.value = this.studentInfo.nickname, i.disabled = !0, t.value = this.studentInfo.number, t.disabled = !0, this.generateReport()) : (this.shadowRoot.querySelector(".initial-form").style.display = "block", this.shadowRoot.querySelector(".report-area").innerHTML = "");
  }
  generateReport() {
    const e = this.shadowRoot.querySelector("#nickname").value, o = this.shadowRoot.querySelector("#student-number").value;
    if (!e || !o) {
      alert("Please enter both nickname and student number.");
      return;
    }
    this.studentInfo = { nickname: e, number: o };
    const i = this.getAttribute("story-title") || "Story Practice", t = (/* @__PURE__ */ new Date()).toLocaleString(), n = this.data.length > 0 ? this.score / this.data.length : 0, s = this.data.length > 0 ? this.recordedSentences.size / this.data.length : 0, r = this.unscrambleData.length || this.unscrambleTotal, l = r > 0 ? this.unscrambleScore / r : 0, a = this.memoryGameData.length / 2 || this.memoryTotal, d = a > 0 ? this.matchedPairsCount / a : 0;
    let p = 85, h = 10, m = 5;
    this.unscrambleData.length === 0 && (p += h, h = 0);
    const c = n * p + l * h + d * m, g = n * (p / 2) + s * (p / 2) + l * h + d * m, b = this.shadowRoot.querySelector(".report-area");
    if (b.innerHTML = `
      <div class="report-card">
        <div class="report-icon">📄</div>
        <h3>Report Card: ${i}</h3>
        <p><strong>Student:</strong> ${e} (${o})</p>
        ...
        ${r > 0 ? `<p><strong>Unscramble Score:</strong> ${this.unscrambleScore} / ${r}</p>` : ""}
  ...
  swapLanguages() {
    const e = this.getAttribute("lang-original") || "en", o = this.getAttribute("lang-translation") || "th";
    this.setAttribute("lang-original", o), this.setAttribute("lang-translation", e), this.isSwapped = !this.isSwapped, this.selectedVoiceName = null, this._updateVoiceList(), this.data = this.data.map((i) => {
      const t = i.fullTranslation, n = i.original, s = i.translationWord, r = i.originalWord;
      return {
        ...i,
        original: t,
        fullTranslation: n,
