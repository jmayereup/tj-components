...
class j extends HTMLElement {
  ...
  _processParsedData(e) {
    this.data = (Array.isArray(e) ? e : [e]).map((r) => {
      ...
      const n = r.original.split(/\s+/), i = r.highlightIndex, a = r.highlightIndexEnd !== void 0 ? r.highlightIndexEnd : i, c = n.slice(i, a + 1).join(" ").replace(/[.,!?;:]/g, "");
      return {
        ...r,
        highlightIndexEnd: a,
        shuffledOptions: o,
  ...
  displayAllLines() {
    ...
    this.data.forEach((t, n) => {
      ...
      a.classList.add("card-header");
      const c = document.createElement("div");
      c.classList.add("original-text"), t.original.split(" ").forEach((h, p) => {
      ...
      }), a.appendChild(c), i.appendChild(a), this.renderLineButtons(n, i);
      const s = document.createElement("div");
      s.classList.add("full-translation"), t.fullTranslation.split(" ").forEach((h, p) => {
        const l = document.createElement("span");
        l.textContent = h + " ", l.classList.add("tts-word"), this.isSwapped && p >= t.highlightIndex && p <= t.highlightIndexEnd && l.classList.add("highlight"), l.onclick = (b) => {
      ...
      const m = this.completedIndices.has(n);
      m && i.classList.add("completed", "answered"), t.shuffledOptions.forEach((h, p) => {
        const l = document.createElement("button");
        l.textContent = h, l.addEventListener("click", () => this.handleSelection(n, p, t.newCorrectIndex, l, i)), m && (l.disabled = !0, p !== t.newCorrectIndex ? l.style.opacity = "0.5" : l.classList.add("success")), d.appendChild(l);
      }), i.appendChild(s), i.appendChild(d), e.appendChild(i);
    }), this.updateProgress();
  }
  _getBestVoice(e) {
  ...
  _updateVoiceList() {
    ...
    let i = e.filter((s) => s.lang.toLowerCase() === t.toLowerCase());
    i.length === 0 && (i = e.filter((s) => s.lang.split(/[-_]/)[0].toLowerCase() === n));
    const a = this.shadowRoot.querySelector(".tj-speed-control");
    if (i.length === 0 || !this._shouldShowAudioControls()) {
      o.style.display = "none", a && (a.style.display = "none");
      return;
    }
    o.style.display = "flex", a && (a.style.display = "inline-flex"), r.innerHTML = "";
    const c = this._getBestVoice(t);
    !this.selectedVoiceName && c && (this.selectedVoiceName = c.name), i.sort((s, d) => s.name.localeCompare(d.name)), i.forEach((s) => {
      const d = document.createElement("button");
      d.classList.add("voice-option-btn"), this.selectedVoiceName === s.name && d.classList.add("active");
      const m = document.createElement("span");
      if (m.textContent = s.name, d.appendChild(m), c && s.name === c.name) {
        const h = document.createElement("span");
        h.classList.add("badge"), h.textContent = "Best", d.appendChild(h);
      }
      d.onclick = () => {
  ...
  _speak(e, r, o = null) {
    ...
    let n = null;
    const i = this.getAttribute("lang-original") || "en", a = r.split(/[-_]/)[0].toLowerCase(), c = i.split(/[-_]/)[0].toLowerCase();
    return this.selectedVoiceName && a === c && (n = window.speechSynthesis.getVoices().find((d) => d.name === this.selectedVoiceName)), n || (n = this._getBestVoice(r)), n && (t.voice = n), t.lang = r, t.rate = this.playbackSpeed, o && (t.onend = o), window.speechSynthesis.speak(t), t;
  }
  toggleFullPlayback() {
  ...
  renderLineButtons(e, r = null) {
    ...
    if (i.classList.add("record-btn"), this.isRecordingLine === e ? (i.classList.add("recording"), i.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><rect x="6" y="6" width="12" height="12"/></svg>', i.onclick = () => this.stopRecording(), i.title = "Stop Recording") : (i.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>', i.onclick = () => this.startRecording(e), i.title = "Record Voice"), t.appendChild(i), this.recordedBlobs.has(e) && this.isRecordingLine !== e) {
      ...
      this.isPlayingRecording === e ? (a.classList.add("play-recorded-btn", "playing"), a.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z"/></svg>') : (a.classList.add("play-recorded-btn"), a.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M10 16.5l6-4.5-6-4.5v9zM12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>'), a.title = "Play Recording", a.onclick = () => this.playRecordedAudio(e), t.appendChild(a);
    }
  }
  playLine(e, r = !1) {
  ...
  handleSelection(e, r, o, t, n) {
    ...
    if (r === o) {
      n.classList.add("answered"), this.answeredCount++, this.completedIndices.add(e), n.dataset.hadError || this.score++, t.classList.add("success"), n.classList.add("completed"), n.classList.remove("failed"), n.querySelectorAll(".translation-options button").forEach((s) => {
        s.disabled = !0, s !== t && (s.style.opacity = "0.5");
      }), this.updateProgress();
      ...
      c && !c.classList.contains("finish-container") ? setTimeout(() => {
        ...
        this.clearPlaybackHighlights();
        const s = this.shadowRoot.querySelector(".finish-btn");
  ...
  startUnscrambleActivity(e = !0) {
    ...
    if (o.length >= 5)
      ...
      t = [...o];
      const i = 5 - t.length, a = r.filter((c) => !o.includes(c)).sort(() => 0.5 - Math.random());
      t = t.concat(a.slice(0, i));
    }
    if (t.forEach((i) => this.unscrambleUsedSentences.add(i)), this.unscrambleUsedSentences.size >= this.data.length && (this.unscrambleUsedSentences.clear(), t.forEach((i) => this.unscrambleUsedSentences.add(i))), this.unscrambleData = t.map((i) => {
      const a = this.data[i], c = a.original.split(/\s+/).filter((d) => d.length > 0), s = [...c].sort(() => 0.5 - Math.random());
      return {
        ...a,
        correctWords: c,
        shuffledWords: s
  ...
  renderUnscrambleChallenge(e = !0) {
    ...
    const i = this.getAttribute("lang-original") || "en", a = document.createElement("button");
    a.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>', a.classList.add("voice-btn"), a.style.margin = "0 auto 1em auto", a.onclick = () => {
    ...
    const s = document.createElement("div");
    s.classList.add("unscramble-result"), s.setAttribute("lang", i), t.appendChild(s);
    const d = document.createElement("div");
    d.classList.add("unscramble-pool"), d.setAttribute("lang", i), t.appendChild(d);
    const m = () => {
      s.innerHTML = "", this.userUnscrambledWords.forEach((g, v) => {
        const y = document.createElement("button");
        y.textContent = g, y.onclick = () => {
          this.userUnscrambledWords.splice(v, 1), m();
        }, s.appendChild(y);
        ...
        if (y < u) {
          ...
          f.textContent = g, f.onclick = () => {
            this.userUnscrambledWords.push(g), m();
          }, d.appendChild(f);
        }
      });
    };
    ...
    p.textContent = "Skip", p.style.opacity = "0.7", p.onclick = () => {
    ...
    l.textContent = "Check", l.classList.add("finish-btn"), l.style.padding = "0.8em 2em", l.onclick = () => {
      this.userUnscrambledWords.join(" ") === o.correctWords.join(" ") ? (this.unscrambleScore++, l.textContent = "Correct! Next", l.classList.add("success"), l.onclick = () => {
        this.currentUnscrambleIndex++, this.userUnscrambledWords = [], this.currentUnscrambleIndex < this.unscrambleData.length ? (this.renderUnscrambleChallenge(), this.updateProgress()) : this.renderUnscrambleCompletion();
      }) : (l.classList.add("error"), setTimeout(() => l.classList.remove("error"), 500));
    };
    const b = document.createElement("button");
    b.textContent = "Reset", b.onclick = () => {
      this.userUnscrambledWords = [], m();
    }, h.appendChild(b), h.appendChild(p), h.appendChild(l), t.appendChild(h), r.appendChild(t), e && t.scrollIntoView({ behavior: "smooth", block: "center" });
  }
  startMemoryGame(e = !0) {
    ...
    const n = [];
    o.forEach((i, a) => {
      n.push({ id: a, type: "original", text: i.original, lang: this.getAttribute("lang-original") || "en" }), n.push({ id: a, type: "translation", text: i.translation, lang: this.getAttribute("lang-translation") || "th" });
  ...
  renderMemoryGameUI() {
    ...
    o.classList.add("memory-grid"), this.memoryGameData.forEach((a, c) => {
      const s = document.createElement("div");
      s.classList.add("memory-card"), s.dataset.index = c, s.innerHTML = `
        <div class="memory-card-inner">
          <div class="memory-card-front">?</div>
          <div class="memory-card-back" lang="${a.lang}">${a.text}</div>
        </div>
      `, s.onclick = () => this.handleMemoryCardFlip(s, a), o.appendChild(s);
    }), e.appendChild(o);
  ...
  generateReport() {
    ...
    const a = this.unscrambleData.length || this.unscrambleTotal, c = a > 0 ? this.unscrambleScore / a : 0, s = this.memoryGameData.length / 2 || this.memoryTotal, d = s > 0 ? this.matchedPairsCount / s : 0;
    let m = 85, h = 10, p = 5;
    this.unscrambleData.length === 0 && (m += h, h = 0);
    const l = i * m + c * h + d * p, b = this.shadowRoot.querySelector(".report-area"), g = l >= 80 ? "🏆" : l >= 50 ? "⭐" : "💪", v = l >= 80 ? "Excellent!" : l >= 50 ? "Good effort!" : "Keep practicing!";
    if (b.innerHTML = `
      <div class="rc-header">
      ...
      <div class="rc-details">
          ...
          ${a > 0 ? `<div class="rc-detail-row"><span>Unscramble Score</span><span>${this.unscrambleScore} / ${a}</span></div>` : ""}
          <div class="rc-detail-row"><span>Matching Pairs</span><span>${this.matchedPairsCount} / ${s}</span></div>
  ...
  async _submitScore() {
    ...
    const n = this.getAttribute("story-title") || "Story Practice", i = this.data.length > 0 ? this.score / this.data.length : 0, a = this.unscrambleData.length || this.unscrambleTotal, c = a > 0 ? this.unscrambleScore / a : 0, s = this.memoryGameData.length / 2 || this.memoryTotal, d = s > 0 ? this.matchedPairsCount / s : 0;
    let m = 85, h = 10, p = 5;
    this.unscrambleData.length === 0 && (m += h, h = 0);
    const l = i * m + c * h + d * p, b = {
  ...
  swapLanguages() {
    ...
    o && o.classList.toggle("active", !this.isSwapped), t && t.classList.toggle("active", this.isSwapped), this.data = this.data.map((n) => {
      const i = n.fullTranslation, a = n.original, c = n.translationWord, s = n.originalWord;
      return {
        ...n,
        original: i,
        fullTranslation: a,
