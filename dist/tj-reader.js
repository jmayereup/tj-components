...
class R extends HTMLElement {
  get code() {
    return this.getAttribute("code") !== null ? this.getAttribute("code") : L.teacherCode;
  }
  set code(e) {
  ...
  constructor() {
    ...
    super(), this.attachShadow({ mode: "open" }), this.shadowRoot.innerHTML = `
      <style>${M}</style>
      ${I}
    `, this.data = [], this.studentInfo = { nickname: "", number: "", homeroom: "", teacherCode: "" }, this.submissionUrl = ((t = L) == null ? void 0 : t.submissionUrl) || "https://script.google.com/macros/s/AKfycbzqV42jFksBwJ_3jFhYq4o_d6o7Y63K_1oA4oZ1UeWp-M4y3F25r0xQ-Kk1n8F1uG1Q/exec", this.isSubmitting = !1, this.score = 0, this.answeredCount = 0, this.isPlayingAll = !1, this.playbackIndex = 0, this.isPaused = !1, this.playbackUtterance = null;
    const e = parseFloat(localStorage.getItem("tj-reader-speed"));
    this.playbackSpeed = isNaN(e) ? 0.7 : e, this.isAutoplay = !0, this.unscrambleData = [], this.currentUnscrambleIndex = 0, this.unscrambleScore = 0, this.userUnscrambledWords = [], this.memoryGameData = [], this.flippedCards = [], this.matchedPairsCount = 0, this.matchingGamesCompleted = 0, this.isCheckingMatch = !1, this.isSwapped = !1, this.selectedVoiceName = null, this.recordedBlobs = /* @__PURE__ */ new Map(), this.recordedSentences = /* @__PURE__ */ new Set(), this.completedIndices = /* @__PURE__ */ new Set(), this.unscrambleCompleted = !1, this.memoryCompleted = !1, this.unscrambleTotal = 0, this.memoryTotal = 0, this.isRecordingLine = null, this.mediaRecorder = null, this.recordingStartTime = 0, this.isPlayingRecording = null, this.shadowRoot.querySelector(".generate-btn").onclick = () => this.generateReport(), this.shadowRoot.querySelector("#play-pause-btn").onclick = () => this.toggleFullPlayback(), this.shadowRoot.querySelector("#stop-btn").onclick = () => this.stopFullPlayback(), this.shadowRoot.querySelector("#voice-btn").onclick = () => this._showVoiceOverlay(), this.shadowRoot.querySelector(".close-voice-btn").onclick = () => this._hideVoiceOverlay(), this.shadowRoot.querySelector(".voice-overlay").onclick = (n) => {
      n.target.classList.contains("voice-overlay") && this._hideVoiceOverlay();
    ...
    r.onchange = (n) => {
      this.isAutoplay = n.target.checked;
    };
    const o = this.shadowRoot.querySelector("#speed-select");
    o && (o.value = this.playbackSpeed.toString(), o.onchange = (n) => {
      this.playbackSpeed = parseFloat(n.target.value), localStorage.setItem("tj-reader-speed", n.target.value);
    });
  }
  connectedCallback() {
  ...
  loadData() {
    try {
      ...
      if (e) {
        const r = e.replace(/"((?:\\.|[^"\\])*)"/gs, (t, n) => '"' + n.replace(/\n/g, "\\n").replace(/\r/g, "\\r") + '"'), o = JSON.parse(r);
  ...
  _processParsedData(e) {
    this.data = (Array.isArray(e) ? e : [e]).map((r) => {
      ...
      const n = r.original.split(/\s+/), i = r.highlightIndex, s = r.highlightIndexEnd !== void 0 ? r.highlightIndexEnd : i, c = n.slice(i, s + 1).join(" ").replace(/[.,!?;:]/g, "");
      return {
        ...r,
        highlightIndexEnd: s,
        shuffledOptions: o,
  ...
  displayAllLines() {
    ...
    this.data.forEach((t, n) => {
      ...
      i.classList.add("card"), i.dataset.index = n;
      ...
      if (c.classList.add("original-text"), r.split(/[-_]/)[0].toLowerCase() === "th" && typeof Intl < "u" && Intl.Segmenter) {
        ...
        for (const l of p)
          if (l.isWordLike) {
            ...
            b.textContent = l.segment, b.classList.add("tts-word"), b.onclick = (g) => {
        ...
        t.original.split(" ").forEach((h, p) => {
          ...
          l.textContent = h + " ", l.classList.add("tts-word"), !this.isSwapped && p >= t.highlightIndex && p <= t.highlightIndexEnd && l.classList.add("highlight"), l.onclick = (b) => {
            b.stopPropagation(), this.isPlayingAll && this.stopFullPlayback(), this._speak(h.replace(/[.,!?;:]/g, ""), r);
          }, c.appendChild(l);
        });
      s.appendChild(c), i.appendChild(s), this.renderLineButtons(n, i);
      ...
      const y = this.completedIndices.has(n);
      y && i.classList.add("completed", "answered"), t.shuffledOptions.forEach((h, p) => {
        const l = document.createElement("button");
        l.textContent = h, l.addEventListener("click", () => this.handleSelection(n, p, t.newCorrectIndex, l, i)), y && (l.disabled = !0, p !== t.newCorrectIndex ? l.style.opacity = "0.5" : l.classList.add("success")), m.appendChild(l);
      }), i.appendChild(d), i.appendChild(m), e.appendChild(i);
    }), this.updateProgress();
  }
  _getBestVoice(e) {
  ...
  _updateVoiceList() {
    ...
    let i = e.filter((a) => a.lang.toLowerCase() === t.toLowerCase());
    i.length === 0 && (i = e.filter((a) => a.lang.split(/[-_]/)[0].toLowerCase() === n));
    const s = this.shadowRoot.querySelector(".tj-speed-control");
    if (i.length === 0 || !this._shouldShowAudioControls()) {
    ...
    !this.selectedVoiceName && c && (this.selectedVoiceName = c.name), i.sort((a, d) => a.name.localeCompare(d.name)), i.forEach((a) => {
      const d = document.createElement("button");
      d.classList.add("voice-option-btn"), this.selectedVoiceName === a.name && d.classList.add("active");
      const u = document.createElement("span");
      if (u.textContent = a.name, d.appendChild(u), c && a.name === c.name) {
  ...
  _speak(e, r, o = null) {
    ...
    let n = null;
    const i = this.getAttribute("lang-original") || "en", s = r.split(/[-_]/)[0].toLowerCase(), c = i.split(/[-_]/)[0].toLowerCase();
    return this.selectedVoiceName && s === c && (n = window.speechSynthesis.getVoices().find((d) => d.name === this.selectedVoiceName)), n || (n = this._getBestVoice(r)), t.lang = r, n && (t.voice = n), t.rate = this.playbackSpeed, o && (t.onend = o), window.speechSynthesis.speak(t), t;
  }
  toggleFullPlayback() {
  ...
  startUnscrambleActivity(e = !0) {
    ...
    if (o.length >= 5)
      ...
      t = [...o];
      const i = 5 - t.length, s = r.filter((c) => !o.includes(c)).sort(() => 0.5 - Math.random());
      t = t.concat(s.slice(0, i));
    }
    if (t.forEach((i) => this.unscrambleUsedSentences.add(i)), this.unscrambleUsedSentences.size >= this.data.length && (this.unscrambleUsedSentences.clear(), t.forEach((i) => this.unscrambleUsedSentences.add(i))), this.unscrambleData = t.map((i) => {
      const s = this.data[i], c = s.original.split(/\s+/).filter((d) => d.length > 0), a = [...c].sort(() => 0.5 - Math.random());
      return {
        ...s,
        correctWords: c,
        shuffledWords: a
  ...
  renderUnscrambleChallenge(e = !0) {
    ...
    const i = this.getAttribute("lang-original") || "en", s = document.createElement("button");
    ...
    const u = () => {
      a.innerHTML = "", this.userUnscrambledWords.forEach((l, b) => {
        const g = document.createElement("button");
        g.textContent = l, g.onclick = () => {
          this.userUnscrambledWords.splice(b, 1), u();
        }, a.appendChild(g);
      }), d.innerHTML = "", o.shuffledWords.forEach((l, b) => {
        ...
        if (g < f) {
          ...
          x.textContent = l, x.onclick = () => {
            this.userUnscrambledWords.push(l), u();
          }, d.appendChild(x);
        }
      });
    };
  ...
  startMemoryGame(e = !0) {
    ...
    const n = [];
    o.forEach((i, s) => {
      n.push({ id: s, type: "original", text: i.original, lang: this.getAttribute("lang-original") || "en" }), n.push({ id: s, type: "translation", text: i.translation, lang: this.getAttribute("lang-translation") || "th" });
  ...
  generateReport() {
    ...
    const t = this.getAttribute("story-title") || "Story Practice", n = (/* @__PURE__ */ new Date()).toLocaleString(), i = this.data.length > 0 ? this.score / this.data.length : 0;
    ...
    let u = 85, m = 10, y = 5;
    this.unscrambleData.length === 0 && (u += m, m = 0);
    const h = i * u + c * m + d * y, p = this.shadowRoot.querySelector(".report-area"), l = h >= 80 ? "🏆" : h >= 50 ? "⭐" : "💪", b = h >= 80 ? "Excellent!" : h >= 50 ? "Good effort!" : "Keep practicing!";
    if (p.innerHTML = `
      <div class="rc-header">
      ...
      <div class="rc-score-row">
          ...
          <div class="rc-score-label">${l} ${b}</div>
      </div>
    ...
    `, this.recordedBlobs.size > 0) {
      ...
      C.classList.add("recordings-list"), Array.from(this.recordedBlobs.keys()).sort((w, S) => w - S).forEach((w) => {
        ...
        j.classList.add("recording-text"), j.textContent = S.original, k.appendChild(v), k.appendChild(j), C.appendChild(k);
  ...
  async _submitScore() {
    ...
    const n = this.getAttribute("story-title") || "Story Practice", i = this.data.length > 0 ? this.score / this.data.length : 0, s = this.unscrambleData.length || this.unscrambleTotal, c = s > 0 ? this.unscrambleScore / s : 0, a = this.memoryGameData.length / 2 || this.memoryTotal, d = a > 0 ? this.matchedPairsCount / a : 0;
    let u = 85, m = 10, y = 5;
    this.unscrambleData.length === 0 && (u += m, m = 0);
    const h = i * u + c * m + d * y, p = {
  ...
  swapLanguages() {
    ...
    o && o.classList.toggle("active", !this.isSwapped), t && t.classList.toggle("active", this.isSwapped), this.data = this.data.map((n) => {
      const i = n.fullTranslation, s = n.original, c = n.translationWord, a = n.originalWord;
      return {
        ...n,
        original: i,
        fullTranslation: s,
  ...
  render() {
    ...
    if (o && (o.textContent = this.getLanguageName(e)), t && (t.textContent = this.getLanguageName(r)), this.displayAllLines(), this.startUnscrambleActivity(!1), this.startMemoryGame(!1), this.updateProgress(), this._updateVoiceList(), !this._shouldShowAudioControls()) {
      const n = this.shadowRoot.querySelector("#autoplay-container");
