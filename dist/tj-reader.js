...
class R extends HTMLElement {
  ...
  _processParsedData(e) {
    this.data = (Array.isArray(e) ? e : [e]).map((r) => {
      ...
      const n = r.original.split(/\s+/), i = r.highlightIndex, s = r.highlightIndexEnd !== void 0 ? r.highlightIndexEnd : i, l = n.slice(i, s + 1).join(" ").replace(/[.,!?;:]/g, "");
      return {
        ...r,
        highlightIndexEnd: s,
        shuffledOptions: o,
  ...
  displayAllLines() {
    ...
    this.data.forEach((t, n) => {
      ...
      l.classList.add("original-text"), t.original.split(" ").forEach((h, m) => {
        ...
        c.textContent = h + " ", c.classList.add("tts-word"), !this.isSwapped && m >= t.highlightIndex && m <= t.highlightIndexEnd && c.classList.add("highlight"), c.onclick = (u) => {
          u.stopPropagation(), this.isPlayingAll && this.stopFullPlayback(), this._speak(h.replace(/[.,!?;:]/g, ""), r);
        }, l.appendChild(c);
      }), s.appendChild(l), i.appendChild(s), this.renderLineButtons(n, i);
      ...
      p && i.classList.add("completed", "answered"), t.shuffledOptions.forEach((h, m) => {
        ...
        c.textContent = h, c.addEventListener("click", () => this.handleSelection(n, m, t.newCorrectIndex, c, i)), p && (c.disabled = !0, m !== t.newCorrectIndex ? c.style.opacity = "0.5" : c.classList.add("success")), d.appendChild(c);
  ...
  _updateVoiceList() {
    ...
    if (i.length === 0 && (i = e.filter((l) => l.lang.split(/[-_]/)[0].toLowerCase() === n)), i.length === 0) {
      o.style.display = "none";
      return;
    }
    o.style.display = "flex", r.innerHTML = "";
    const s = this._getBestVoice(t);
    !this.selectedVoiceName && s && (this.selectedVoiceName = s.name), i.sort((l, a) => l.name.localeCompare(a.name)), i.forEach((l) => {
  ...
  _speak(e, r, o = null) {
    ...
    let n = null;
    const i = this.getAttribute("lang-original") || "en", s = r.split(/[-_]/)[0].toLowerCase(), l = i.split(/[-_]/)[0].toLowerCase();
    return this.selectedVoiceName && s === l && (n = window.speechSynthesis.getVoices().find((d) => d.name === this.selectedVoiceName)), n || (n = this._getBestVoice(r)), n && (t.voice = n), t.lang = r, t.rate = 0.7, o && (t.onend = o), window.speechSynthesis.speak(t), t;
  }
  toggleFullPlayback() {
  ...
  startUnscrambleActivity(e = !0) {
    ...
    if (o.length >= 5)
      ...
      t = [...o];
      const i = 5 - t.length, s = r.filter((l) => !o.includes(l)).sort(() => 0.5 - Math.random());
    ...
    if (t.forEach((i) => this.unscrambleUsedSentences.add(i)), this.unscrambleUsedSentences.size >= this.data.length && (this.unscrambleUsedSentences.clear(), t.forEach((i) => this.unscrambleUsedSentences.add(i))), this.unscrambleData = t.map((i) => {
      const s = this.data[i], l = s.original.split(/\s+/).filter((d) => d.length > 0), a = [...l].sort(() => 0.5 - Math.random());
      return {
        ...s,
        correctWords: l,
        shuffledWords: a
  ...
  renderUnscrambleChallenge(e = !0) {
    ...
    const p = () => {
      a.innerHTML = "", this.userUnscrambledWords.forEach((g, v) => {
        ...
        const y = this.userUnscrambledWords.filter((f) => f === g).length, b = o.shuffledWords.filter((f) => f === g).length;
        if (y < b) {
    ...
    }, h.appendChild(u), h.appendChild(m), h.appendChild(c), t.appendChild(h), r.appendChild(t), e && t.scrollIntoView({ behavior: "smooth", block: "center" });
  }
  startMemoryGame(e = !0) {
  ...
  generateReport() {
    ...
    const s = this.unscrambleData.length || this.unscrambleTotal, l = s > 0 ? this.unscrambleScore / s : 0, a = this.memoryGameData.length / 2 || this.memoryTotal, d = a > 0 ? this.matchedPairsCount / a : 0;
    let p = 85, h = 10, m = 5;
    this.unscrambleData.length === 0 && (p += h, h = 0);
    const c = i * p + l * h + d * m, u = this.shadowRoot.querySelector(".report-area"), g = c >= 80 ? "🏆" : c >= 50 ? "⭐" : "💪", v = c >= 80 ? "Excellent!" : c >= 50 ? "Good effort!" : "Keep practicing!";
    if (u.innerHTML = `
      <div class="rc-header">
  ...
  async _submitScore() {
    ...
    const n = this.getAttribute("story-title") || "Story Practice", i = this.data.length > 0 ? this.score / this.data.length : 0, s = this.unscrambleData.length || this.unscrambleTotal, l = s > 0 ? this.unscrambleScore / s : 0, a = this.memoryGameData.length / 2 || this.memoryTotal, d = a > 0 ? this.matchedPairsCount / a : 0;
    let p = 85, h = 10, m = 5;
    this.unscrambleData.length === 0 && (p += h, h = 0);
    const c = i * p + l * h + d * m, u = {
  ...
  swapLanguages() {
    ...
    o && o.classList.toggle("active", !this.isSwapped), t && t.classList.toggle("active", this.isSwapped), this.data = this.data.map((n) => {
      const i = n.fullTranslation, s = n.original, l = n.translationWord, a = n.originalWord;
      return {
        ...n,
        original: i,
        fullTranslation: s,
