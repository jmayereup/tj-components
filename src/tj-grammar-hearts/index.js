...
class TjGrammarHearts extends HTMLElement {
  ...
  async _submitScore() {
    ...
    if (currentTeacherCode !== this.code) {
      this.submissionError = 'Invalid or missing Submit Code. Please take a screenshot of this report and show it to your teacher instead.';
      this.render();
      return;
    }
  ...
  render() {
    ...
    if (this.gameState === 'hint') {
      ...
      const q = this.currentPool[this.currentIndex];
      const progressPct = ((this.currentIndex + 1) / this.currentPool.length) * 100;
      content = `
        <div class="header">
